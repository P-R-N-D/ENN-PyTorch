...
try:
    ...
    from torchrl.data import TensorSpec, UnboundedDiscreteTensorSpec, CompositeSpec
    from tensordict import TensorDict
except Exception:  # TorchRL not installed
    EnvBase = None


if EnvBase is not None:

    class TorchEnvironment(EnvBase):
        """Minimal EnvBase implementation compatible with TorchRL collectors.

        Observations are a dictionary with key 'observation'. Rewards are scalar.
        Replace the dynamics with your task logic as needed.
        """

        batch_locked = False

        def __init__(self, obs_shape=(1,), action_space=2, device=None):
            super().__init__(device=device, batch_size=())
            self._count = 0
            self.observation_spec = CompositeSpec(
                observation=TensorSpec(
                    shape=torch.Size(obs_shape), dtype=torch.float32, device=self.device
                )
            ).to(self.device)
            self.action_spec = UnboundedDiscreteTensorSpec(
                n=action_space, shape=torch.Size(()), device=self.device
            )
            self.reward_spec = TensorSpec(
                shape=torch.Size(()), dtype=torch.float32, device=self.device
            )
            self.done_spec = TensorSpec(
                shape=torch.Size(()), dtype=torch.bool, device=self.device
            )

        def _reset(self, tensordict: Optional[TensorDict] = None) -> TensorDict:
            obs = torch.zeros(
                self.observation_spec["observation"].shape,
                dtype=torch.float32,
                device=self.device,
            )
            td = TensorDict(
                {
                    "observation": obs,
                    "done": torch.zeros((), dtype=torch.bool, device=self.device),
                },
                batch_size=[],
                device=self.device,
            )
            ...
            td = TensorDict(
                {
                    ...
                    "done": torch.tensor(self._count >= 1, dtype=torch.bool, device=self.device),
                },
                batch_size=[],
                device=self.device,
            )
        ...
        def make_collector(
            ...
            except Exception as exc:  # pragma: no cover - torchrl optional
                raise RuntimeError("torchrl is not available") from exc

            def _env_maker():
                return self

            return MultiSyncDataCollector(
                create_env_fn=_env_maker,
                policy=policy,
                frames_per_batch=frames_per_batch,
                total_frames=total_frames,
                collector_kwargs={"num_workers": num_workers},
            )
