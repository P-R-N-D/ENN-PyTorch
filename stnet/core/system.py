...
def _ensure_importable_main_for_spawn() -> Optional[str]:
    ...
    try:
        ...
        tmp_dir = "/tmp"
    stub_path = os.path.join(tmp_dir, "stnet_mp_main.py")
    try:
        if not os.path.isfile(stub_path):
            with open(stub_path, "w", encoding="utf-8") as f:
                f.write(
                    "# Auto-generated by stnet to make multiprocessing "
                    "spawn/forkserver work in interactive environments.\n"
...
def optimal_start_method() -> str:
    ...
    if os.name == "posix" and current == "forkserver":
        return "forkserver"
    if platform.startswith("win") and current == "spawn":
        return "spawn"
...
def init_start_method() -> None:
    ...
    if os.name == "posix":
        _ensure_importable_main_for_spawn()
    if platform.startswith("win"):
        if existing == "spawn":
            return
        candidates = ("spawn",)
...
def get_device(
    ...
    **kwargs: Any,
    ...
    if is_accelerator_available("cuda"):
        ...
        cuda_b = getattr(torch.backends, "cuda", None)
        matmul = (
            getattr(cuda_b, "matmul", None) if cuda_b is not None else None
        )
