...
from .pipeline import Dataset, Source, SourceType, _FEATURE_KEY_ALIASES, _LABEL_KEY_ALIASES
...
class Storage:
    get_meta_path = staticmethod(get_meta_path)
    write_json = staticmethod(write_json)
    save_temp = staticmethod(save_temp)
...
class Prefetcher(Buffer):
    ...
    def __iter__(self) -> Iterator[Any]:
        ...
        fut = ex.submit(
            ...
            use_host_pool=bool(self._pin_pool),
            host_pool=self._host_pool,
            accel_stream=self._accel_stream,
            accel_event_pool=self._accel_event_pool,
            guard_ttl_s=float(self._guard_ttl_s),
        )
        try:
            while True:
                try:
