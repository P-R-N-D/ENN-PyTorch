...
def _size_of_retnet(x: torch.Tensor, blk0: nn.Module, *args: Any, mode: str) -> int:
    if not isinstance(x, torch.Tensor) or x.dim() != 3: return 0
    B, L, D = map(int, x.shape)
    if B <= 0 or L <= 0 or D <= 0: return 0
    ...
    hid = getattr(ffn, "hidden_dim", 0) or getattr(ffn, "hid", 0) or int(float(D) * 4.0 * (2.0 / 3.0))
...
def _autofit_microbatch(device: torch.device, hard_max: int, per_sample_bytes: int) -> int:
    if hard_max <= 0 or per_sample_bytes <= 0: return 1
    from ..core.system import Memory as _Mem
    try: host_free = int(_Mem.available())
    except: host_free = None
    dev_free = None
    if device.type in {"cuda", "xpu", "mps"}:
        with contextlib.suppress(Exception): dev_free = int(_Mem.mem_get_info(device)[0])
    eff = min(host_free, dev_free) if (host_free and dev_free) else (dev_free or host_free)
    if not eff or eff <= 0: return hard_max
...
def _coerce_tensor(t: torch.Tensor, *args, enabled: bool, inplace: bool) -> torch.Tensor:
    return torch.nan_to_num(t, nan=0.0, posinf=0.0, neginf=0.0) if enabled and (t.is_floating_point() or t.is_complex()) else t


def _prealloc_microbatch(inp: torch.Tensor, microbatch: int, run_fn: Callable, *args, pad_to: Optional[int] = None, out_dtype: Optional[torch.dtype] = None, cast_slice: Optional[Callable] = None, stage: str = "microbatch"):
    if inp.ndim < 1: raise ValueError(f"{stage}: expected batched input, got {inp.shape}")
    ...
    if pad_i is not None and (pad_i := max(1, pad_i)) < mb_i: raise ValueError(f"{stage}: pad_to {pad_i} < microbatch {mb_i}")
    out_bufs: Optional[List[torch.Tensor]] = None
    pad_buf = None
    for s in range(0, total_b, mb_i):
        x_slice = inp[s : s + mb_i]
        if cast_slice: x_slice = cast_slice(x_slice)
        ...
        if pad_i is not None and slice_n < pad_i:
            ...
            if not torch.is_grad_enabled():
                if pad_buf is None or pad_buf.shape != want_shape or pad_buf.dtype != x_slice.dtype or pad_buf.device != x_slice.device:
                     pad_buf = x_slice.new_empty(want_shape)
                pad_buf.zero_(); pad_buf[:slice_n].copy_(x_slice)
                x_in = pad_buf
            else:
                x_in = x_slice.new_zeros(want_shape); x_in[:slice_n].copy_(x_slice)
            did_pad = True
        out = run_fn(x_in)
        outs = (out,) if torch.is_tensor(out) else out
        if not outs: raise RuntimeError(f"{stage}: empty output at s={s}")
        processed: List[torch.Tensor] = []
        for j, t in enumerate(outs):
            if not torch.is_tensor(t): raise TypeError(f"{stage}: output #{j} invalid")
            y = t[:slice_n] if did_pad else t
            if y.shape[0] != slice_n: raise RuntimeError(f"{stage}: output size mismatch")
            if out_dtype and y.dtype != out_dtype: y = y.to(dtype=out_dtype)
            processed.append(y)
        if out_bufs is None:
            out_bufs = [y.new_empty((total_b, *y.shape[1:])) for y in processed]
        else:
            if len(out_bufs) != len(processed): raise RuntimeError(f"{stage}: arity mismatch")
            for k, (buf, y) in enumerate(zip(out_bufs, processed)):
                if buf.shape[1:] != y.shape[1:]: raise RuntimeError(f"{stage}: shape mismatch output#{k}")
        for buf, y in zip(out_bufs, processed):
            buf[s : s + slice_n].copy_(y)
    if not out_bufs: raise RuntimeError(f"{stage}: no outputs")
...
def stochastic_depth_schedule(drop_path: float, depth: int) -> List[float]:
    if depth <= 0: return []
    if drop_path <= 0.0 or depth == 1: return [float(drop_path) if depth==1 else 0.0] * depth
...
class CrossTransformer(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        if cross is None:
            ...
            if len(cross_mods) != 2: raise ValueError(f"CrossTransformer expects 2 modules, got {len(cross_mods)}")
    ...
    def forward(
        ...
        if spatial_tokens.dim() != 3 or temporal_tokens.dim() != 3: raise ValueError("Expects 3D tensors")
        if not torch.jit.is_tracing():
            if spatial_tokens.size(0) != temporal_tokens.size(0): raise ValueError("Batch mismatch")
            if spatial_tokens.size(2) != temporal_tokens.size(2): raise ValueError("Hidden dim mismatch")
        mode_l = _coerce_modeling_types(self._fixed_mode or mode or "st")

        def _impl(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
            if mode_l == "ss": return self.cross[0](a, b)
            if mode_l == "tt": return self.cross[1](b, a)
            s_context = self.cross[0](a, b)
            t_context = self.cross[1](b, a)
            base_s = torch.cat([s_context, t_context.mean(dim=1, keepdim=True).expand_as(s_context)], dim=-1)
            fused_s = self.mix(self.mix_norm(base_s))
            out_s = s_context + self.drop_path(self.dropout(fused_s))
            base_t = torch.cat([t_context, s_context.mean(dim=1, keepdim=True).expand_as(t_context)], dim=-1)
        ...
        def _ckpt_impl(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
            if torch.is_grad_enabled(): _unshard_fsdp_module(self)
            return _impl(a, b)

        if self.training and torch.is_grad_enabled() and not is_export_or_trace():
            return coerce_checkpoint(_ckpt_impl, spatial_tokens, temporal_tokens, use_reentrant=True, preserve_rng_state=True)
...
class LongNet(nn.Module):
    ...
    def _should_enable_checkpoint(self, out, layout_batch_first, need_weights, key_padding_mask):
        if not (self.training and torch.is_grad_enabled() and not need_weights and self._ckpt_enabled and not is_export_or_trace()):
            return False
        try:
            B, L, D = (out.shape[0], out.shape[1], out.shape[2]) if layout_batch_first else (out.shape[1], out.shape[0], out.shape[2])
            ...
            for lyr in self.layers:
                dense = not (out.is_cuda and (flex or ((getattr(lyr, "dilation", 1) == 1) and getattr(lyr, "window_size", None) is None and key_padding_mask is None)))
                scores = (int(B) * int(H) * int(L) * int(L) * score_b) if dense else 0
                hid = 0
                if (ffn := getattr(lyr, "ffn", None)) and isinstance(ffn, nn.Sequential) and len(ffn) > 0:
    ...
    def _ckpt_fn(self, t, layer, key_padding_mask):
        if torch.is_grad_enabled(): _unshard_fsdp_module(self); _unshard_fsdp_module(layer)
        return layer(t, key_padding_mask=key_padding_mask, need_weights=False, average_attn_weights=False, skip_ffn_checkpoint=True)[0]
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        do_ckpt = self._should_enable_checkpoint(out, layout_batch_first, need_weights, key_padding_mask)
        for layer in self.layers:
            if do_ckpt:
                out = coerce_checkpoint(self._ckpt_fn, out, layer, key_padding_mask, use_reentrant=True, preserve_rng_state=True)
                attn_w = None
            else:
                out, attn_w = layer(
                    out,
