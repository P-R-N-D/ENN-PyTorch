...
| resource monitoring, cost/batching, runtime governor, and orchestration | `enn_torch_dev/runtime/cost.py`, `footprint.py`, `resources.py`, `batching.py`, `governor.py`, `orchestration.py` | `enn_torch_dev/debug/runtime -q` | `docs/CURRENT_STATE.md`, `docs/TESTING.md`, `docs/RUNTIME_SAFETY.md`, relevant `docs/dev_*.md` | CPU/CUDA assumptions, memory accounting semantics, oversized batch behavior, budget shrink/grow bounds, accidental execution/retry coupling, unbounded pass materialization |
