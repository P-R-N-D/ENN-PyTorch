    ...
    *args: Any,
    ...
    try:
        with inference_mode(run_model), StatelessAutocast.float(device):
            ...
            partial_broadcast_fp32_match_frac = float(
                env_float("ENN_PRED_PARTIAL_BROADCAST_FP32_MATCH_FRAC", 0.97)
            )
            partial_broadcast_fp32_min_gain = float(
                env_float("ENN_PRED_PARTIAL_BROADCAST_FP32_MIN_GAIN", 0.01)
            )
            partial_broadcast_fp32_min_ymax_gain = float(
                env_float("ENN_PRED_PARTIAL_BROADCAST_FP32_MIN_YMAX_GAIN", 0.0)
            )
            partial_broadcast_fp32_persist = bool(
                env_bool(
                    "ENN_PRED_PARTIAL_BROADCAST_FP32_PERSIST",
                    default=False,
                )
            )
                                    ...
                                    with contextlib.suppress(Exception):
                                        if int(preds.shape[0]) >= 2:
                                            ...
                                            partial_like2 = bool(
                                                ...
                                                and (mf2 >= float(partial_broadcast_fp32_match_frac))
