    ...
    with join_context:
        ...
        for epoch_idx in range(int(total_epochs)):
            ...
            if (
                ...
                _LOGGER.info(
                    "[swa] epoch %d: update+ckpt (CPU shadow)", epoch_idx + 1
                )
                swa_target = (
                    model.module
                    if isinstance(
...
def process(*args: Any, **kwargs: Any) -> object:
    ...
    if ops.mode == "train":
        ...
        if local_rank == 0:
            ...
            if (
                swa_helper is not None
                and getattr(swa_helper, "n_averaged", 0) > 0
            ):
                avg_tag = "swa"
