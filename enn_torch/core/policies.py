...
from .precision import DeviceMeta, Quantization, StatelessAutocast, is_scale_safe
from .system import (
    CPU,
    _call,
...
with contextlib.suppress(Exception):
    ...
    _FLEX_HAS_SCORE_MOD = "score_mod" in set(inspect.signature(_fa).parameters.keys())
...
class WorkerPolicy:
    ...
    def optimize(
        ...
        if device is not None:
            ...
            try:
                dev_obj = device if isinstance(device, torch.device) else torch.device(str(device))
        ...
        local_world_guess = max(
            1,
            int(env_first_int(("ENN_LOCAL_WORLD_SIZE", "LOCAL_WORLD_SIZE", "SLURM_NTASKS_PER_NODE"), local_world_guess)),
        )

        if is_accel and int(nacc) > 0:
            allow_over = int(env_first_int(("ENN_ALLOW_ACCELERATOR_OVERSUBSCRIBE", "ENN_ALLOW_GPU_OVERSUBSCRIBE"), 0))
        ...
        try:
            thr = int(env_first_int(("ENN_THREAD_CAP_THRESHOLD_CORES", "ENN_THREAD_CAP_THRESHOLD"), 8))
            low_mult = int(env_first_int(("ENN_THREAD_CAP_MULT_LOW", "ENN_THREAD_CAP_MULT_SMALL"), 2))
            high_mult = int(env_first_int(("ENN_THREAD_CAP_MULT_HIGH", "ENN_THREAD_CAP_MULT_LARGE"), 1))
            cap_mult = max(1, int(low_mult)) if int(ncpu_raw) <= int(thr) else max(1, int(high_mult))
        except Exception:
            pass
        ...
        thread_cap_total = _optimal_threads(
            ncpu=ncpu_raw, cap_mult=cap_mult, local_world=int(local_world_guess), distribute=bool(distribute)
        )

        compile_threads = 0
        if env_bool("ENN_TORCH_COMPILE", default=True):
            requested = int(env_first_int(("ENN_INDUCTOR_COMPILE_THREADS", "ENN_COMPILE_THREADS"), 0))
            if requested <= 0:
            ...
            if requested > 0:
                ...
                compile_threads = 1 if int(thread_cap_total) <= 6 else 2

        min_non_compile = 3
        max_compile = max(0, int(thread_cap_total) - int(min_non_compile))
        compile_threads = 0 if max_compile <= 0 else max(0, min(int(compile_threads), int(max_compile)))

        thread_cap_after_compile = max(2, int(thread_cap_total) - int(compile_threads))
        eff_cores_pre = max(1, int(thread_cap_after_compile) // max(1, int(cap_mult)))

        ckpt_writer_threads = int(env_first_int(("ENN_DCP_WRITER_THREADS", "ENN_CKPT_WRITER_THREADS"), 0) or 0)
        if ckpt_writer_threads <= 0:
            match eff_cores_pre:
                case c if c <= 4: ckpt_writer_threads = 1
                case c if c <= 8: ckpt_writer_threads = 2
                case _: ckpt_writer_threads = 4

        min_non_ckpt = 2
        ckpt_writer_threads = max(0, min(int(ckpt_writer_threads), max(0, int(thread_cap_after_compile) - int(min_non_ckpt))))
        thread_cap = max(2, int(thread_cap_after_compile) - int(ckpt_writer_threads))
        ...
        soft_inflight_max = max(8, env_first_int(("ENN_SOFT_INFLIGHT_MAX",), soft_inflight_max_default))
        ...
        if is_accel:
            match eff_cores:
                case c if c <= 4: model_ratio = 1.00
                case c if c <= 8: model_ratio = 0.90
                case c if c <= 16: model_ratio = 0.80
                case c if c <= 32: model_ratio = 0.70
                case _: model_ratio = 0.60
        else:
            match eff_cores:
                case c if c <= 4: model_ratio = 1.00
                case c if c <= 8: model_ratio = 0.95
                case c if c <= 16: model_ratio = 0.90
                case _: model_ratio = 0.85

        with contextlib.suppress(Exception):
            env_key = "ENN_MODEL_CORE_RATIO_ACCEL" if is_accel else "ENN_MODEL_CORE_RATIO"
        ...
        match model_budget:
            case b if b <= 2: inter_ops = 1
            case b if b <= 8: inter_ops = max(1, b // 4)
            case b: inter_ops = max(2, min(8, b // 6))
        ...
        base_workers = min(max(1, int(data_budget)), int(thread_cap), int(soft_inflight))
        max_workers = max(1, int((int(soft_inflight) - int(prebatch)) // max(1, int(prefetch_factor))))
        ...
        return cls(
            nproc_per_node=int(local_world_guess),
            device=dev_type,
            local_world_size=int(local_world_guess),
    ...
    def set_thread_setting(self: Self) -> None:
        ...
        if ct > 0:
            explicit = env_str("ENN_INDUCTOR_COMPILE_THREADS") or env_str("ENN_COMPILE_THREADS")
            if explicit is None:
        ...
        if wt > 0:
            explicit = env_str("ENN_DCP_WRITER_THREADS") or env_str("ENN_CKPT_WRITER_THREADS")
            if explicit is None:
...
class LoaderPolicy:
    ...
    def hard_inflight_batches(self: Self, device: Union[torch.device, str]) -> int:
    ...
    def apply_soft_limits(self: Self, wp: WorkerPolicy, device: Union[torch.device, str]) -> WorkerPolicy:
        ...
        with contextlib.suppress(Exception):
            wp.max_concurrency = max(
                1, min(int(getattr(wp, "max_concurrency", 1) or 1), int(wp.num_workers) if int(wp.num_workers) > 0 else 1, int(soft_cap))
            )
        return wp

    def wrap_input(self: Self, loader: Any, device: Union[torch.device, str], *args: Any, name: str) -> Any:
...
class BatchPolicy:
    ...
    def suggest_batch(
        ...
        *args: Any,
        ...
        use_dev_budget = (self.device_budget_ratio > 0.0 or self.device_budget_min_bytes > 0 or self.device_budget_max_bytes is not None)
        use_host_budget = (self.host_budget_ratio > 0.0 or self.host_budget_min_bytes > 0 or self.host_budget_max_bytes is not None)
        ...
        if dev_free is not None and dev_free >= 0 and self.sample_bytes > 0:
            ...
            if use_dev_budget:
                budget = self._budget_bytes(
                    dev_total, budget_ratio=self.device_budget_ratio, budget_min_bytes=self.device_budget_min_bytes, budget_max_bytes=self.device_budget_max_bytes
                )
                if budget > 0:
        ...
        if host_free is not None and host_free >= 0 and (self.host_sample_bytes or 0) > 0:
            ...
            if use_host_budget:
                budget = self._budget_bytes(
                    host_total, budget_ratio=self.host_budget_ratio, budget_min_bytes=self.host_budget_min_bytes, budget_max_bytes=self.host_budget_max_bytes
                )
                if budget > 0:
                    usable = min(int(usable), int(budget))
        ...
        return max(1, max(int(b), int(self.min_batch)))
...
class ModelPolicy:
    ...
    @staticmethod
    def _align_layers(src: nn.Module, dst: nn.Module, params_dtype: Optional[torch.dtype]) -> None:
    ...
    @staticmethod
    def _nvidia_linear(module: nn.Linear, params_dtype: Optional[torch.dtype], te: Any) -> Optional[nn.Module]:
    ...
    @staticmethod
    def _nvidia_layer_norm(module: nn.LayerNorm, params_dtype: Optional[torch.dtype], te: Any) -> Optional[nn.Module]:
    ...
    @staticmethod
    def _nvidia_rms_norm(module: nn.Module, params_dtype: Optional[torch.dtype], te: Any) -> Optional[nn.Module]:
    ...
    def _to_nvidia_attention(
        ...
        **kwargs: Any,
        ...
        for module in model.modules():
            if dot_cls is not None and isinstance(module, dot_cls) and getattr(module, "_te_ok", False):
    ...
    def _enable_nvidia_training(
        model: nn.Module, params_dtype: torch.dtype, logger: Optional[Callable[[str], None]]
    ) -> Tuple[nn.Module, bool, str]:
        try:
            swapped_model, n = ModelPolicy._to_nvidia_layers(
                ...
                apply_te_layer_norm=True,
                apply_te_rms_norm=True,
                filter_linear=lambda lyr, _: lyr.in_features % 16 == 0 and lyr.out_features % 16 == 0,
                params_dtype=params_dtype,
            )
            if n > 0:
    ...
    def _enable_torchao_training(
        model: nn.Module, logger: Optional[Callable[[str], None]]
    ) -> Tuple[nn.Module, bool, str]:
        try:
    ...
    def _enable_nvidia_inference(
        ...
        try:
            ...
            if n > 0:
                ...
                if logger:
                    logger(f"[FP8][TE] swapped {n} modules; using te.autocast")
    ...
    @staticmethod
    def _reuse_nvidia_layers(model: nn.Module, logger: Optional[Callable[[str], None]]) -> Tuple[nn.Module, bool, str]:
        te_present = any((getattr(module.__class__, "__module__", "").startswith("transformer_engine") for module in model.modules()))
        if te_present:
    ...
    def _enable_torchao_inference(
        model: nn.Module, dynamic_activations: bool, logger: Optional[Callable[[str], None]]
    ) -> Tuple[nn.Module, bool, str]:
        try:
            dev = None
            with contextlib.suppress(Exception):
            ...
            if (not bool(ok)) or ("fp8-ok:ao" not in r):
                return (model, False, f"AO skipped (fp8 not validated for AO): {reason}")
            ...
            def _eligible_linear(m: nn.Module) -> bool:
                if getattr(m, "_enn_no_ao_quant", False) or not isinstance(m, nn.Linear):
                    return False
                inf = getattr(m, "in_features", None)
                outf = getattr(m, "out_features", None)
                if inf is None or outf is None:
                    return False
                inf, outf = int(inf), int(outf)
                return inf >= 16 and outf >= 16 and (inf % 16) == 0 and (outf % 16) == 0
            ...
            if sig is not None and ("filter_fn" in sig.parameters):
                ...
                for m in model.modules():
                    if isinstance(m, nn.Linear):
                        ...
                        if inf < 16 or outf < 16 or (inf % 16) != 0 or (outf % 16) != 0:
                            risky = True
                            break
                if risky:
                    return (model, False, "AO skipped (quantize_ has no filter_fn; risky Linear present)")
                quantize_(model, cfg)
            ...
            if wrapped == 0 and float8_param_tensors == 0 and float8_linear_weights == 0 and float8_buffer_tensors == 0:
                return (model, False, "AO quantize produced no torchao float8 modules/tensors")
    ...
    def quantize_for_float8_training(
        model: nn.Module, metadata: Optional[Dataset[Any]] = None, logger: Optional[Callable[[str], None]] = None
    ) -> Tuple[nn.Module, bool, str]:
        from ..data.pipeline import Dataset
        ...
        if isinstance(reason, str) and ("te-only" in reason.lower()):
            _log_info(logger, f"[FP8][quantize] TE-only fallback enabled (torchao missing): {reason}")
            
        if getattr(meta, "has_scale", False):
            float8_dtypes = StatelessAutocast.float8_formats()
            p2 = env_int("ENN_FP8_SCALE_HEADROOM_POW2", env_int("ENN_FP8_HEADROOM_POW2", 1))
            try:
                p2 = int(p2)
            except Exception:
                p2 = 1
            ...
            margin = float(2 ** p2)
            if not any(is_scale_safe(dtype, meta, safety_margin=margin) for dtype in float8_dtypes):
                _log_info(logger, f"[FP8][quantize] training disabled: data scale exceeds float8 range (headroom=2^{p2})")
                StatelessAutocast.configure(model, metadata=meta)
                return (model, False, "data scale")
                
        params_dtype = ModelPolicy.negotiate(device, metadata=meta)
        allow_ao = bool(env_bool("ENN_FP8_ALLOW_AO_TRAINING", default=env_bool("ENN_FP8_ALLOW_AO", default=False)))
        ...
        for backend in backends:
            ...
            if ok2:
                _log_info(logger, f"[FP8][quantize] training enabled via {why} ({reason})")
        ...
        StatelessAutocast.configure(model, metadata=meta)
        return (model, False, "No usable FP8 backend" + (" (torchao disabled)" if not allow_ao else ""))
    ...
    def quantize_for_float8_prediction(
        ...
        if getattr(meta, "has_scale", False):
            ...
            margin = float(2 ** p2)
            if not any(is_scale_safe(dtype, meta, safety_margin=margin) for dtype in float8_dtypes):
                _log_info(logger, f"[FP8] inference disabled: data scale exceeds float8 range (headroom=2^{p2})")
                StatelessAutocast.configure(model, metadata=meta)
                return (model, False, "data scale")
                
        params_dtype = ModelPolicy.negotiate(device, metadata=meta)
        dynamic_activations = not (getattr(meta, "has_scale", False) and getattr(meta, "scale_is_integral", None) is True)
        allow_ao = bool(env_bool("ENN_FP8_ALLOW_AO_INFERENCE", default=env_bool("ENN_FP8_ALLOW_AO", default=False)))
        order = ("te_swap", "te_present", "ao") if allow_ao else ("te_swap", "te_present")
        ...
        for step in order:
            match step:
                ...
                case _:
                    m2, ok2, why = ModelPolicy._enable_torchao_inference(model, dynamic_activations, logger)
    ...
    def quantize_for_int8_training(
        ...
        m2, ok, why = Quantization.enable_qat(
            model, dynamic_activations=dynamic_activations, group_size=group_size, logger=logger
        )
    ...
    def quantize_for_int8_prediction(
        ...
        dynamic_activations = not (getattr(meta, "has_scale", False) and getattr(meta, "scale_is_integral", None) is True)
        m2, ok, why = Quantization._enable_ptq(model, dynamic_activations=dynamic_activations, logger=logger)
        StatelessAutocast.configure(m2 if ok else model, metadata=meta)
        return (m2, ok, why)
...
class PrecisionPolicy:
    ...
    def from_metadata(
        ...
        *args: Any,
        ...
        meta = metadata if metadata is not None else DeviceMeta.for_device(dev)
        ...
        amp_dtype: Optional[torch.dtype] = None
        fp32_safe = bool(has_scale) and is_scale_safe(torch.float32, meta, safety_margin=safety)
    ...
    def to_fsdp_policy(self: Self) -> Any:
...
class CollectivePolicy:
    ...
    def from_env(cls: type[Self]) -> "CollectivePolicy":
        backend = env_str("ENN_COLLECTIVE_BACKEND") or env_str("ENN_BCAST_BACKEND") or "c10d"
        return cls(
            backend=backend.strip().lower(),
            include_parameters=env_bool(("ENN_COLLECTIVE_INCLUDE_PARAMETERS", "ENN_BCAST_INCLUDE_PARAMETERS"), default=True),
            include_buffers=env_bool(("ENN_COLLECTIVE_INCLUDE_BUFFERS", "ENN_BCAST_INCLUDE_BUFFERS"), default=True),
            max_buffer_size_mb=env_first_int(("ENN_COLLECTIVE_MAX_BUFFER_SIZE_MB", "ENN_BCAST_MAX_BUFFER_SIZE_MB"), default=25),
            coalesce_mb=env_first_int(("ENN_COLLECTIVE_COALESCE_MB", "ENN_BCAST_COALESCE_MB"), default=64),
            max_tensor_mb_for_coalesce=env_first_int(("ENN_COLLECTIVE_MAX_TENSOR_MB_FOR_COALESCE", "ENN_BCAST_MAX_TENSOR_MB_FOR_COALESCE"), default=8),
            inter_stream_mb=env_first_int(("ENN_COLLECTIVE_INTER_STREAM_MB", "ENN_BCAST_INTER_STREAM_MB"), default=16),
            intra_stream_mb=env_first_int(("ENN_COLLECTIVE_INTRA_STREAM_MB", "ENN_BCAST_INTRA_STREAM_MB"), default=64),
            max_inflight_mb=env_first_int(("ENN_COLLECTIVE_MAX_INFLIGHT_MB", "ENN_BCAST_MAX_INFLIGHT_MB"), default=64),
            debug_collectives=env_bool(("ENN_COLLECTIVE_DEBUG", "ENN_BCAST_DEBUG"), default=False),
            verbose=env_bool(("ENN_COLLECTIVE_VERBOSE", "ENN_BCAST_VERBOSE"), default=False),
        )
...
class DistributedPolicy:
    ...
    def from_env(cls: type[Self]) -> "DistributedPolicy":
        return cls(
            prefer_hsdp=env_bool("ENN_DISTRIBUTED_PREFER_HSDP", default=True),
            prefer_ddp=env_bool("ENN_DISTRIBUTED_PREFER_DDP", default=True),
            sync_state=env_bool("ENN_DISTRIBUTED_SYNC_STATE", default=True),
    ...
    def plan(
        ...
        *,
        ...
        if q.dtype == torch.float64:
            ...
            allow_flex_fp8 = bool(env_bool("ENN_FLEX_ALLOW_FP8", default=False))
            is_fp8 = self._is_float8_dtype(q.dtype) or self._is_torchao_float8_tensor(q)
            allow_fp32 = bool(env_bool("ENN_FLEX_ALLOW_FP32", default=True))
            flex_ok = (
                bool(allow_flex)
                and (not env_bool("ENN_DISABLE_FLEX_ATTENTION", default=False))
                and _HAS_TORCH_FLEX
                ...
                and (q.dtype in (torch.float16, torch.bfloat16) or (q.dtype == torch.float32 and allow_fp32))
            )
        ...
        if forced_be is not None:
            match forced_be:
                case AttentionBackend.FLEX if flex_ok:
                    return AttentionPlan(
                        AttentionBackend.FLEX,
                        use_score_mod_for_bias=bool(has_bias and _FLEX_HAS_SCORE_MOD),
                        reason="forced:flex",
                    )
                case AttentionBackend.MHA if mha_ok:
                    return AttentionPlan(AttentionBackend.MHA, reason="forced:mha")
                case AttentionBackend.DPA if dpa_ok:
