...
from typing import Any, Callable, Dict, Iterator, List, Optional, Self, Tuple, Union
...
# 3. Local Imports
# =============================================================================
from ..core.datatypes import default_underflow_action, env_bool, normalize_underflow_action
...
_TORCHAO_IMPORT_TRIED: bool = False
...
# =============================================================================
# Internal Environment & Lazy Loading Helpers
# =============================================================================
...
def _import_torchao_quantization() -> None:
    global _Int8DynamicActivationInt8WeightConfig, _Int8WeightOnlyConfig
    global _PTQ_IMPL, _qp, _TORCHAO_IMPORT_TRIED
...
# =============================================================================
# Type Formatting & Tree Manipulation Helpers
# =============================================================================
...
def _coerce_torch_dtype(value: Any, default: torch.dtype) -> torch.dtype:
    match value:
        ...
        case None:
            return default
        case str():
            return getattr(torch, value.strip().replace("torch.", ""), default)
        case _:
            return getattr(torch, str(value).strip().replace("torch.", ""), default)
...
def _get_meta_stats(meta: Any | None) -> Dict[str, Any]:
    if not meta:
        return {}
    keys = (
        "has_scale", "has_nonfinite", "scale_max_abs", "scale_min_positive",
        "scale_is_integral", "scale_min_value", "scale_max_value",
        "underflow_action", "int_quant_bits"
    )
    return {k: _to_serializable(getattr(meta, k, None)) for k in keys}
...
def _validate_dtype_safety(
    ...
    **kwargs: Any,
    ...
    if max_abs is None:
        mn, mx = getattr(meta, "scale_min_value", None), getattr(meta, "scale_max_value", None)
    ...
    if getattr(dtype, "is_complex", False):
        ...
        ok, why = _validate_dtype_safety(base_dtype, meta, safety_margin=safety_margin, underflow_action=action)
    ...
    if getattr(dtype, "is_floating_point", False):
        ...
        if action == "forbid" and (mp := getattr(meta, "scale_min_positive", None)) is not None:
            ...
            if math.isfinite(mp_f) and mp_f > 0.0 and mp_f < uf_limit:
...
def is_scale_safe(
    ...
    **kwargs: Any,
    ...
    ok, _ = _validate_dtype_safety(dtype, meta, safety_margin=safety_margin, underflow_action=underflow_action)
    return ok
        ...
        for mod in getattr(model, "modules", lambda: [])():
            ...
            if params:
                for _name, p in list(params.items()):
                    if p is None or not isinstance(p, torch.Tensor): continue
                    if is_meta_or_fake_tensor(p): continue
                    if (not p.is_floating_point()) or p.dtype == dtype: continue
                    try:
            ...
            if bufs:
                for _name, b in list(bufs.items()):
                    if b is None or not isinstance(b, torch.Tensor): continue
                    if is_meta_or_fake_tensor(b): continue
                    if (not b.is_floating_point()) or b.dtype == dtype: continue
                    if b.dtype is torch.float64 and dtype is not torch.float64: continue
                    try:
    ...
    with torch.no_grad():
        for mod in getattr(module, "modules", lambda: [])():
            if is_precision_exempted(mod):
                continue
            match mod:
                case nn.BatchNorm1d() | nn.BatchNorm2d() | nn.BatchNorm3d() | nn.SyncBatchNorm():
...
def get_layernorm_dtype(device: torch.device | str) -> torch.dtype:
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            ...
            mf = PrecisionPolicy.from_metadata(device=dev, metadata=meta, logger=_LOGGER).master_float
            if mf == torch.float64:
        ...
        chosen = StatelessAutocast.negotiate(
            tuple(cands), fallback=torch.get_default_dtype(), context="cpu.layernorm", device=dev, meta=meta
        )
...
def preload_layers(model: nn.Module, device: torch.device | str) -> None:
    ...
    for module in model.modules():
        ...
        if dev.type == "cpu":
            target_dtype = get_layernorm_dtype(dev)
        else:
            target_dtype = None
            for tensor in (weight, bias):
                if isinstance(tensor, torch.Tensor) and tensor.is_floating_point() and not is_meta_or_fake_tensor(tensor):
        ...
        if module.elementwise_affine:
            if not isinstance(weight, torch.Tensor) or is_meta_or_fake_tensor(weight):
                data = torch.ones(module.normalized_shape, device=dev, dtype=target_dtype)
            ...
            if not isinstance(bias, torch.Tensor) or is_meta_or_fake_tensor(bias):
                data = torch.zeros(module.normalized_shape, device=dev, dtype=target_dtype)
        ...
        if dev.type == "cpu":
            if isinstance(weight, torch.Tensor) and weight.dtype != target_dtype:
                data = weight.to(device=dev, dtype=target_dtype)
                _set_requires_grad(module, "weight", data, requires_grad=requires_grad_w)
            if isinstance(bias, torch.Tensor) and bias.dtype != target_dtype:
                data = bias.to(device=dev, dtype=target_dtype)
                _set_requires_grad(module, "bias", data, requires_grad=requires_grad_b)
        elif (
            isinstance(weight, torch.Tensor) and isinstance(bias, torch.Tensor)
            and weight.is_floating_point() and bias.is_floating_point()
            and (bias.dtype != weight.dtype)
        ):
            data = bias.to(device=dev, dtype=weight.dtype)
            _set_requires_grad(module, "bias", data, requires_grad=requires_grad_b)
...
def validate_model_dtype_unity(model: nn.Module, device: torch.device | str) -> None:
    ...
    for name, module in model.named_modules():
        ...
        if expected is not None and dev.type != "cpu":
            dtypes = {
            ...
            if len(dtypes) > 1:
                module_name = name or module.__class__.__name__
                mismatches.append(f"{module_name} parameters disagree on dtype: {sorted(str(d) for d in dtypes)}")
...
def _iter_tensors(x: Any) -> Iterator[torch.Tensor]:
    match x:
        case _ if torch.is_tensor(x):
            yield x
        case Mapping():
            for v in x.values():
...
class StatelessAutocast:
    ...
    def _resolve_backend(
        cls: type[Self], preferred: Optional[str], device: torch.device, kind: str
    ) -> Optional[str]:
        ok_te, ok_ao = False, False
        match kind:
            case "fp8":
                ...
                match (ok_te, ok_ao):
                    case (True, True): order = ("te", "ao")
                    case (True, False): order = ("te",)
                    case (False, True): order = ("ao",)
                    case _: order = ()
            case _:
                order = ("ao",)
        ...
        for backend in order:
            match backend:
                case "te":
                    if cls._try_load_backend("transformer_engine.pytorch", "autocast", ok_te if kind == "fp8" else ok, f"Autocast {kind.upper()} TE unavailable: {why}"):
                ...
                case "ao":
                    ...
                    if cls._try_load_backend(mod, attr, ok_ao if kind == "fp8" else ok, f"Autocast {kind.upper()} torchao unavailable: {why}"):
    ...
    @classmethod
    def _fp8_backend(
        cls: type[Self], pref: Optional[str], *args: object, device: Optional[torch.device] = None, **kwargs: object
    ) -> Optional[str]:
        if _env_disable_fp8():
            return None
        match str(pref).strip().lower():
    ...
    @classmethod
    def _nvidia_float8(cls: type[Self], device: torch.device, enabled: bool) -> List[AbstractContextManager[None]]:
        if not enabled:
            return []
    ...
    @classmethod
    def _torchao_float8(cls: type[Self], enabled: bool) -> List[AbstractContextManager[None]]:
        return cls._get_backend_context("torchao.float8", "fp8_autocast", enabled)

    @classmethod
    def _torchao_int8_backend(cls: type[Self], device: torch.device, enabled: bool) -> List[AbstractContextManager[None]]:
        if cls._preferred_int_backend is None:
            cls._preferred_int_backend = "ao"
        return cls._get_backend_context("torchao.quantization", "int8_autocast", enabled)

    @classmethod
    def _torchao_int4(cls: type[Self], device: torch.device, enabled: bool) -> List[AbstractContextManager[None]]:
        return cls._get_backend_context("torchao.quantization", "int4_autocast", enabled)

    @classmethod
    def _torchao_int8(cls: type[Self], device: torch.device, enabled: bool) -> List[AbstractContextManager[None]]:
        return cls._torchao_int8_autocast(device, enabled)

    @classmethod
    def _torchao_int8_autocast(cls: type[Self], device: torch.device, enabled: bool) -> List[AbstractContextManager[None]]:
        if cls._preferred_int_backend is None:
            cls._preferred_int_backend = "ao"
    ...
    @classmethod
    def coerce_metadata(
        cls: type[Self], device: Optional[Union[torch.device, str]] = None, *args: Any, metadata: Any | None = None, **kwargs: Any
    ) -> Any:
        ...
        if meta is None:
            ...
            if current_device != dev:
                ...
                if callable(ensure := getattr(meta, "ensure_device_info", None)):
                    with contextlib.suppress(Exception): ensure()
                    
        if not getattr(meta, "float_dtypes", ()):
            if callable(refresh := getattr(meta, "refresh", None)):
                with contextlib.suppress(Exception): refresh()
    ...
    @classmethod
    def float_amp_priority(cls: type[Self], device: torch.device) -> Tuple[torch.dtype, ...]:
        meta = cls.coerce_metadata(device)
        candidates = getattr(meta, "float_dtypes", ())
        return tuple(candidates) if candidates else (torch.float32,)

    @classmethod
    def integer_amp_priority(cls: type[Self], device: torch.device) -> Tuple[torch.dtype, ...]:
        meta = cls.coerce_metadata(device)
        candidates = getattr(meta, "int_dtypes", ())
        return tuple(candidates) if candidates else (torch.int64,)
    ...
    def configure(
        ...
        *args: Any,
        ...
        backend = "off" if _env_disable_fp8() else fp8_backend
        int_b = int_backend or (
            "ao" if isinstance(model, nn.Module) and any(
                getattr(model, a, False) for a in ("__int8_training_qat__", "__int8_training_ptq__", "__int8_inference_ao__")
            ) else None
        )
        if backend is None and isinstance(model, nn.Module):
            if any(getattr(model, a, False) for a in ("__fp8_training_te__", "__fp8_inference_te__", "__te_fp8_default__")):
                backend = "te"
            elif any(getattr(model, a, False) for a in ("__fp8_training_ao__", "__fp8_inference_ao__")):
                backend = "ao"
    ...
    def negotiate(
        ...
        **kwargs: Any,
        ...
        underflow_override = normalize_underflow_action(
            kwargs.pop("underflow_action", kwargs.pop("underflow", None)), default=default_underflow_action()
        )
        collect_checks = bool(logger and (logger.isEnabledFor(logging.DEBUG) or logger.isEnabledFor(logging.INFO)))
        ...
        if selected is None:
            ...
            def _add(dt: Any) -> None:
                ...
                if not isinstance(dt, torch.dtype) or dt in seen: return
                seen.add(dt); ordered.append(dt)
        ...
        if logger is not None:
            ...
            if logger.isEnabledFor(logging.INFO if level == "info" else logging.DEBUG):
                scale_key = (
                    getattr(meta, "has_scale", False), getattr(meta, "has_nonfinite", False),
                    getattr(meta, "scale_max_abs", None), getattr(meta, "underflow_action", None),
                )
                decision_key = decision_key or ("amp", str(context), str(device), tuple(_parse_dtype(x) for x in candidates), _parse_dtype(fallback), scale_key, float(safety_margin), int(safety_margin_pow2))
                payload: dict[str, object] = {
                    "context": str(context), "device": str(device),
                    "selected": _parse_dtype(selected), "selected_from": selected_from,
                    "scale": _get_meta_stats(meta),
                }
                if fallback_order: payload["fallback_order"] = [_parse_dtype(x) for x in fallback_order]
    ...
    def resolve_float_dtype(
        cls: type[Self], device: Optional[Union[torch.device, str]] = None, dtype: Optional[Union[torch.dtype, str]] = None, metadata: Any | None = None
    ) -> Optional[torch.dtype]:
        dev = cls._device(device)
        meta = cls.coerce_metadata(device=dev, metadata=metadata)
        disable = False
        if meta is not None:
            if callable(fn := getattr(meta, "is_disabled", None)):
                with contextlib.suppress(Exception): disable = bool(fn())
        if disable:
            return None
        ...
        return cls.negotiate(candidates, fallback=requested_dtype, device=dev, meta=meta, context="autocast-float")
    ...
    def float(
        ...
        amp_candidates = tuple(getattr(meta, "float_dtypes", ()) or ()) if meta is not None else ()
        if not amp_candidates:
            amp_candidates = (cls._last_float_dtype,)
            
        amp_dtype = cls.negotiate(
            tuple(amp_candidates), fallback=cls._last_float_dtype, logger=_LOGGER, context="float", device=dev, meta=meta
        )
        ...
        if wants_fp8:
            for b in (backend, "ao") if backend == "te" else (backend,):
                ctxs = cls._nvidia_float8(dev, True) if b == "te" else cls._torchao_float8(True)
                if ctxs:
        ...
        requested_dtype = cls._last_float_dtype if cls._last_float_dtype in amp_candidates and cls._last_float_dtype == amp_dtype else amp_dtype
        ...
        if _LOGGER.isEnabledFor(logging.DEBUG):
            with contextlib.suppress(Exception):
                _LOGGER.debug(
                    ...
                    json.dumps(
                        {"device": str(dev), "amp_dtype": _parse_dtype(amp_dtype), "fp8_used": fp8_backend_used, "scale": _get_meta_stats(meta)},
    ...
    @classmethod
    @contextlib.contextmanager
    def suspend(cls: type[Self], device: Optional[Union[torch.device, str]] = None) -> Iterator[None]:
        dev = cls._device(device)
        with contextlib.ExitStack() as stack:
            with contextlib.suppress(Exception):
                stack.enter_context(torch.amp.autocast(device_type=dev.type, enabled=False))
            with contextlib.suppress(Exception):
                if callable(te_autocast := getattr(importlib.import_module("transformer_engine.pytorch"), "autocast", None)):
                    stack.enter_context(te_autocast(enabled=False))
            with contextlib.suppress(Exception):
                if callable(fp8_ac := getattr(importlib.import_module("torchao.float8"), "fp8_autocast", None)):
            ...
            if _LOGGER.isEnabledFor(logging.DEBUG):
                with contextlib.suppress(Exception):
                    _LOGGER.debug(
                        ...
                        json.dumps(
                            {"device": str(dev), "int_dtype": _parse_dtype(int_dtype), "int_backend_used": int_backend_used, "scale": _get_meta_stats(meta)},
                            sort_keys=True, default=str
                        )
                    )
            yield
...
class Quantization:
    ...
    @staticmethod
    def _prepare_qat(
        model: nn.Module, *args: Any, dynamic_activations: bool = True, group_size: int = 128, logger: Optional[Callable[[str], None]] = None, **kwargs: Any
    ) -> Any:
        _ = args, kwargs
        _import_torchao_quantization()
        if _qp is None:
        ...
        try:
            from torchao.quantization.fake_quant import FakeQuantizeConfig, Int8ActivationConfig, Int8WeightConfig, prepare_qat_
    ...
    def _apply_ptq(
        ...
        _ = args, kwargs
        _import_torchao_quantization()
        if _PTQ_IMPL is None or _Int8DynamicActivationInt8WeightConfig is None:
    ...
    @classmethod
    def enable_qat(
        cls: type[Self], model: nn.Module, *args: Any, dynamic_activations: bool = True, group_size: int = 128, logger: Optional[Callable[[str], None]] = None, **kwargs: Any
    ) -> tuple[nn.Module, bool, str]:
        _ = args, kwargs
        if not cls.is_qat_available():
            return (model, False, "QAT backend unavailable")
        try:
            cls._prepare_qat(model, dynamic_activations=dynamic_activations, group_size=group_size, logger=logger)
    ...
    def _enable_ptq(
        ...
        _ = args, kwargs
        return cls._apply_ptq(model, dynamic_activations=dynamic_activations, group_size=group_size, logger=logger)
    ...
    def enable_int8_training(
        ...
        _ = args, kwargs
        if getattr(model, "__int8_training_qat__", False) or getattr(model, "__int8_training_ptq__", False):
        ...
        try:
            m2, ok, why = cls._apply_ptq(model, dynamic_activations=dynamic_activations, group_size=group_size, logger=logger)
        except Exception as exc:
...
class AutocastState:
    ...
    def _norm_policy(self) -> str:
        match str(self.policy or "").strip().lower():
            case "cooldown" | "cd":
                return "cooldown"
            case _:
                return "sticky"
    ...
    def tick(self) -> None:
        ...
        if self._norm_policy() == "cooldown" and int(self._disable_until) > 0 and int(self.step) >= int(self._disable_until):
            self._disabled = False
            self._disable_until = 0

    def should_disable_amp(self) -> bool:
        match self._norm_policy():
            case "sticky":
    ...
    def record_failure(self) -> None:
        ...
        if self._norm_policy() == "cooldown":
            self._disable_until = int(self.step) + max(0, int(self.cooldown_steps))
