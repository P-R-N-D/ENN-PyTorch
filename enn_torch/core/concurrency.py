...
# =============================================================================
# Protocols, Data Structures & Exceptions
# =============================================================================
...
# =============================================================================
# Internal Helpers: Environment & Configuration
# =============================================================================
...
def _get_throttle_state() -> str:
    s = str(
        env_first(
            ("ENN_CACHE_BACKPRESSURE_MODE", "ENN_CACHE_BACKPRESSURE", "ENN_CACHE_MODE"),
            default="block",
        ) or "block"
    ).strip().lower()
...
def _get_throttle_timeout() -> float:
    val = env_first_float(
        ("ENN_CACHE_BACKPRESSURE_TIMEOUT_S", "ENN_CACHE_SUBMIT_TIMEOUT_S"), default=0.05
    ) or 0.05
    return max(0.0, float(val))

def _is_early_release_enabled() -> bool:
    return bool(env_flag("ENN_CACHE_EARLY_RELEASE", "ENN_CACHE_RELEASE_EARLY", default=True))
...
def _is_affinity_strict() -> bool:
    return bool(env_flag("ENN_EXECUTOR_AFFINITY_STRICT", "ENN_AFFINITY_STRICT", default=False))
...
def _set_concurrency_env(key: str, value: str, *args: Any, force: bool, cap_down: bool = True) -> None:
    try:
        ...
        if not cap_down:
            return
        try:
            cur_i, tgt_i = int(str(cur).strip()), int(str(v).strip())
            if cur_i > tgt_i:
...
def _limit_inner_threads(threads: int, *args: Any, force: bool = False) -> int:
    ...
    force = bool(force) or bool(env_flag("ENN_EXECUTOR_FORCE_INNER_THREADS", default=False))
    ...
    interop = max(1, int(env_first_int(("ENN_EXECUTOR_INTEROP_THREADS",), default=1) or 1))
...
def _max_outer_concurrency() -> int:
    return int(env_first_int(("ENN_EXECUTOR_OUTER_CONCURRENCY", "ENN_EXECUTOR_OUTER_LIMIT"), default=0) or 0)

def _outer_concurrency_mode() -> str:
    s = str(env_first(("ENN_EXECUTOR_OUTER_CONCURRENCY_MODE",), default="auto") or "auto").strip().lower()
    match s:
...
def _outer_concurrency_limit(wl: str, executor_kind: str, nlogical: int, nphysical: int, mw: int) -> int:
    wl = str(wl or "").strip().lower()
    if wl not in {"cpu", "compute"} or str(executor_kind) not in {"thread", "interpreter"}:
        return 0
    ...
    nlog, nphy = max(1, int(nlogical)), max(1, int(nphysical))
    
    match _outer_concurrency_mode():
        case "logical":
            base = nlog
        case "physical":
            base = nphy
...
# =============================================================================
# Internal Helpers: Topology & Affinity
# =============================================================================
...
def _executor_scatter_cpus(cpus: Sequence[int]) -> list[int]:
    ...
    secondary: list[int] = [int(x) for g in groups for x in g if int(x) not in primary]
    allowed = set(base)
...
def _pick_cores_balanced(cpus: Sequence[int], start: int, idx: int, mw: int, cpw: int, salt: int) -> list[int]:
    base = list(cpus or [])
    n = len(base)
    if n <= 0:
        return []
...
def _set_current_thread_affinity(cores: Sequence[int]) -> None:
    if cores and sys.platform.startswith("linux"):
        with contextlib.suppress(Exception):
...
def _executor_thread_initializer(role: str, wl: str, cpus: Sequence[int], start: int, mw: int, cpw: int, salt: int) -> None:
...
def _executor_process_initializer(wl: str, cpus: Sequence[int], start: int, mw: int, cpw: int, salt: int) -> None:
...
class Mutex:
    ...
    def acquire(self: Self, blocking: bool = True, timeout: float | None = None) -> bool:
        if timeout is None:
    ...
    def __exit__(self: Self, exc_type: type[BaseException] | None, exc: BaseException | None, tb: TracebackType | None) -> None:
        self.release()
...
class BoundedExecutor(futures.Executor):
    ...
    def submit(self: Self, fn: Callable[..., Any], /, *args: Any, **kwargs: Any) -> futures.Future:
        sem = self._sem
        sem.acquire()
        try:
    ...
    def map(self: Self, fn: Callable[..., Any], *iterables: Any, timeout: float | None = None, chunksize: int = 1) -> Any:
        del chunksize
        sem = self._sem
        end_time = (time.monotonic() + float(timeout)) if timeout is not None else None
    ...
    def shutdown(self: Self, wait: bool = True, *args: Any, cancel_futures: bool = False) -> None:
        try:
...
class Thread:
    def __init__(self: Self, io_workers: int, enabled: bool = True, allow_omp_bind: bool = True) -> None:
    ...
    def _import_psutil() -> Optional[ModuleType]:
        ...
        return importlib.import_module("psutil") if spec else None
    ...
    def _retune_threads(self: Self) -> None:
        if not self._enabled or getattr(self._tls, "in_retune", False):
            return
        ...
        try:
            ...
            distribute = bool(env_first_int(("ENN_DISTRIBUTE_THREAD_CAP",), int(distribute_default)))
            
            thread_cap = _optimal_threads(ncpu=cpus, cap_mult=cap_mult, local_world=local_world, distribute=bool(distribute))
                ...
                match sys.platform:
                    ...
                    case "darwin":
                        with contextlib.suppress(Exception):
                            ...
                            ok = (lib.thread_policy_set(port, THREAD_AFFINITY_POLICY, ctypes.byref(policy), 1) == 0)
            case _:
                pass
    ...
    def tune_threads(self: Self, io_workers: Optional[int] = None, *_unused_args: Any, initial: bool = False, **_unused_kwargs: Any) -> None:
        if not self._enabled:
            return
...
class Affinity:
    __slots__ = (
        "_parent", "_fn", "_pin_thread", "_tls", "_lock", "_tune",
        "_sample_every", "_flush_every", "_perf_counter_ns", "_thread_time_ns"
    )

    def __init__(
        self: Self, parent: "Thread", fn: Callable[[Any], Any], pin_thread: Callable[[], None],
        tls: Any, lock: Any, tune: Callable[..., None], sample_every: int, flush_every: int,
        perf_counter_ns: Callable[[], int], thread_time_ns: Optional[Callable[[], int]],
    ) -> None:
        self._parent = parent
        self._fn = fn
...
# =============================================================================
# Memory Management & Spooling Classes
# =============================================================================
class Disposable:
...
class BufferQueue:
    def __init__(self: Self, max_batches: int) -> None:
        ...
        self._warn_blocking = env_flag("ENN_BUFFER_WARN_BLOCKING", "ENN_DEBUG", default=False)
    ...
    def put(self: Self, item: Any, *args: Any, timeout: float | None = None) -> bool:
        ...
        if self._warn_blocking and elapsed > 0.1:
            logging.warning("BufferQueue.put blocked for %.3f s (max_batches=%d)", float(elapsed), int(self.max_batches))
        return True
...
class Prefetcher:
    ...
    def __init__(self: Self, iterable: Any, *args: Any, max_batches: int = 4, name: str = "buffer", daemon: bool = True, _session: bool = False) -> None:
        self._src = iterable
    ...
    def __iter__(self: Self) -> Iterator[Any]:
        if not bool(self._session):
            return iter(Prefetcher(self._src, max_batches=int(self._max_batches), name=self._name, daemon=self._daemon, _session=True))
        return self._iter_session()

    def __getattr__(self: Self, name: str) -> Any:
        return getattr(self._src, name)

    def _producer_loop(self: Self, src: Any, buf: BufferQueue, sentinel: object) -> None:
            ...
            while True:
                ...
                if isinstance(item, ProducerError):
                    if isinstance(item.exc, (KeyboardInterrupt, SystemExit)):
                        raise item.exc
                    raise RuntimeError(f"{self._name} producer crashed: {item.exc}\n{item.tb}") from item.exc
                yield item
        finally:
            buf.stop()
            with contextlib.suppress(Exception): buf.clear()
            with contextlib.suppress(Exception):
                try: fut.result(timeout=float(self._join_timeout_s))
...
class TensorPage:
    ...
    def __init__(self: Self, numel: int, dtype: torch.dtype, *args: Any, pin_memory: bool = True) -> None:
        self._numel = int(max(1, int(numel)))
        self._dtype = dtype
        self._pinned = bool(pin_memory)
        self._buf = torch.empty(self._numel, dtype=self._dtype, device="cpu", pin_memory=bool(self._pinned))

    @property
    def numel(self: Self) -> int: return int(self._numel)
    @property
    def dtype(self: Self) -> torch.dtype: return self._dtype
    @property
    def pinned(self: Self) -> bool: return bool(self._pinned)
...
class TensorPagePool:
    ...
    def __init__(self: Self, capacity: int = 4, *args: Any, pin_memory: bool = True) -> None:
    ...
    def get(self: Self, shape: Tuple[int, ...], dtype: torch.dtype, *args: Any, return_handle: bool = False, block: bool = False, timeout: float | None = None) -> torch.Tensor | tuple[torch.Tensor, TensorPagePool.Token | None]:
        ...
        while True:
            ...
            if grow:
                entry = TensorPagePool._Entry(page=TensorPage(numel=need, dtype=dtype, pin_memory=self._pin), busy=True, fence=None, gen=0)
                with self._cv:
    ...
    def get_like(self: Self, t: torch.Tensor, *args: Any, return_handle: bool = False, block: bool = False, timeout: float | None = None) -> torch.Tensor | tuple[torch.Tensor, TensorPagePool.Token | None]:
        return self.get(tuple(int(s) for s in t.shape), t.dtype, return_handle=return_handle, block=block, timeout=timeout)

    def fence_event(self: Self, token: TensorPagePool.Token | None, factory: Callable[[], object] | None) -> object | None:
        if token is None or factory is None: return None
        i, g = int(getattr(token, "i", -1)), int(getattr(token, "g", -1))
        if i < 0: return None
        with self._cv:
    ...
    def release_after(self: Self, token: TensorPagePool.Token | None, wait_event: object | None) -> None:
...
class TensorSpooler:
    ...
    def _is_cpu_pinned(t: torch.Tensor) -> bool:
        if not torch.is_tensor(t) or getattr(t, "device", None) is None or t.device.type != "cpu":
            return False
        with contextlib.suppress(Exception):
            return bool(t.is_pinned()) if hasattr(t, "is_pinned") else False
        return False

    def _init_tensor(self: Self, tensor: torch.Tensor, *args: Any, release_cb: Callable[[], Any] | None = None, early_release: bool | None = None, force_unpin: bool | None = None) -> tuple[torch.Tensor, bool]:
    ...
    def _save_tensor(self: Self, tensor: torch.Tensor, path: str) -> None:
        ...
        if str(path).endswith(".mmt"):
            ...
            write_json(get_meta_path(path), {"shape": list(map(int, buf.shape)), "dtype": str(buf.dtype).replace("torch.", "")}, indent=None)
            return
            
        if str(path).endswith((".pt", ".pth")): save_temp(path, buf)
        else: torch.save(buf, path)

    def __enter__(self: Self) -> Self: return self
    ...
    def submit(self: Self, tensor: torch.Tensor, path: Optional[str] = None, idx: Optional[int] = None, wait_event: Optional[object] = None, release_cb: Optional[object] = None) -> None:
        if self._err_event.is_set(): raise RuntimeError(f"Async writer error: {self._err!r}")
        if self._closed.is_set(): raise RuntimeError("TensorSpooler is closed")
        ...
        if self._sem:
            ...
            if mode in ("sync", "raise"):
                ...
                if not acquired:
                    if mode == "raise": raise RuntimeError("TensorSpooler queue is full")
                ...
                while not acquired:
                    if self._err_event.is_set() or self._closed.is_set(): raise RuntimeError("TensorSpooler unavailable")
    ...
    def close(self: Self) -> None:
        ...
        with contextlib.suppress(Exception): self._future.result()
        with contextlib.suppress(Exception): self._executor.shutdown(wait=True)

    def had_error(self: Self) -> bool:
        return bool(self._err_event.is_set())

# =============================================================================
# Public Utility & Factory Functions
# =============================================================================
def is_free_threading_build() -> bool:
    with contextlib.suppress(Exception):
        import sysconfig
        v = sysconfig.get_config_var("Py_GIL_DISABLED")
        if v is not None: return bool(int(v))
    with contextlib.suppress(Exception):
...
def is_gil_enabled() -> bool:
    with contextlib.suppress(Exception): return bool(CPU.is_gil_enabled())
    fn = getattr(sys, "_is_gil_enabled", None)
    if callable(fn):
        with contextlib.suppress(Exception): return bool(fn())
    return True
...
def new_prefetcher(iterable: Any, *args: Any, max_batches: int = 4, name: str = "buffer", daemon: bool = True) -> Prefetcher:
    return Prefetcher(iterable, max_batches=max_batches, name=name, daemon=daemon)

def new_executor(max_workers: int, *args: Any, workload: str = "io", name: str = "enn_torch", prefer_interpreters: bool | None = None) -> futures.Executor:
    ...
    if wl in {"cpu", "compute"}:
        if not is_gil_enabled():
            ...
            if prefer_interpreters is None:
                prefer_interpreters = bool(env_flag("ENN_PREFER_INTERPRETER_POOL", default=True))
            ...
            if cpus_scatter:
                ...
                if executor_kind == "process" and wl in {"cpu", "compute"}:
                    ...
                    if _are_processes_limited():
                        ...
                        if tgt > 0: mw_eff = min(int(mw_eff), max(1, int(tgt)))
                        else: mw_eff = min(int(mw_eff), max(1, int(nphysical_for_limit or len(cpus_scatter))))
                    mw_eff = max(1, int(mw_eff))
                    
                match executor_kind:
                    case "process":
                        default_cpw = max(1, int(len(cpus_scatter) // max(1, mw_eff)))
                        cpw = int(env_first_int(("ENN_EXECUTOR_CORES_PER_WORKER_CPU", "ENN_AFFINITY_CORES_PER_WORKER_CPU"), default=default_cpw) or default_cpw)
                        cpw = max(1, min(int(cpw), max(1, len(cpus_scatter))))
                        start = _executor_scope_start(role="process", wl=wl, prefix=thread_prefix, mw=int(mw_eff), cpw=cpw, ordinal=ordinal, ncpu=len(cpus_scatter))
                        init_fn, init_args = _executor_process_initializer, (wl, cpus_scatter, int(start), int(mw_eff), int(cpw), 11)
                    
                    case "interpreter":
                        cpus_pref = _executor_prefer_smt_lane(cpus_scatter, prefer_primary=True)
                        nlogical_for_limit = int(len(cpus_pref))
                        start = _executor_scope_start(role="interpreter", wl=wl, prefix=thread_prefix, mw=int(mw_eff), cpw=1, ordinal=ordinal, ncpu=max(1, len(cpus_pref)))
                        init_fn, init_args = _executor_thread_initializer, ("interpreter", wl, cpus_pref, int(start), int(mw), 1, 23)
                        
                    case _:
                        cpus_pref = _executor_prefer_smt_lane(cpus_scatter, prefer_primary=False)
                        nlogical_for_limit = int(len(cpus_pref))
                        start = _executor_scope_start(role="thread", wl=wl, prefix=thread_prefix, mw=int(mw_eff), cpw=1, ordinal=ordinal, ncpu=max(1, len(cpus_pref)))
                        init_fn, init_args = _executor_thread_initializer, ("thread", wl, cpus_pref, int(start), int(mw), 1, 37)
        except Exception:
            if _is_affinity_strict(): raise
            init_fn, init_args, nlogical_for_limit, nphysical_for_limit, mw_eff = None, (), 0, 0, int(mw)
            
    if executor_kind in {"thread", "interpreter"} and _is_inner_thread_limited(wl):
        _limit_inner_threads(1)
        
    if nlogical_for_limit <= 0:
        with contextlib.suppress(Exception): nlogical_for_limit = int(len(_executor_allowed_cpus()))
    if nphysical_for_limit <= 0:
        nphysical_for_limit = int(nlogical_for_limit or mw)
        
    outer_limit = _outer_concurrency_limit(wl, executor_kind, int(nlogical_for_limit or mw), int(nphysical_for_limit or (nlogical_for_limit or mw)), int(mw))
    mw_outer = int(mw)
    ...
    if executor_kind == "thread":
        ex = futures.ThreadPoolExecutor(max_workers=mw_outer, thread_name_prefix=thread_prefix, initializer=init_fn, initargs=init_args)
    ...
    if executor_kind == "interpreter":
        ...
        if cls is None:
            ex = futures.ThreadPoolExecutor(max_workers=mw_outer, thread_name_prefix=thread_prefix, initializer=init_fn, initargs=init_args)
            return BoundedExecutor(ex, outer_limit) if (outer_limit and outer_limit < mw) else ex
        try: ex = cls(max_workers=mw_outer, thread_name_prefix=thread_prefix, initializer=init_fn, initargs=init_args)
        except TypeError: ex = cls(max_workers=mw_outer, thread_name_prefix=thread_prefix)
        return BoundedExecutor(ex, outer_limit) if (outer_limit and outer_limit < mw) else ex
        
    try: return futures.ProcessPoolExecutor(max_workers=int(mw_eff), initializer=init_fn, initargs=init_args)
    except TypeError: return futures.ProcessPoolExecutor(max_workers=int(mw_eff))
...
def new_affinity(io_workers: Optional[int] = None) -> "Thread":
    ...
    if _TLB_SINGLETON is None:
        with _TLB_SINGLETON_LOCK:
            if _TLB_SINGLETON is None:
                default_workers = int(io_workers) if io_workers is not None else max(1, int(CPU.count()) // 2)
...
def close(obj: Any, *args: Any, join_timeout: float | None = 1.0) -> None:
    for name in ("cleanup", "close", "shutdown", "stop", "terminate", "disconnect", "release", "join"):
        if callable(fn := getattr(obj, name, None)):
            with contextlib.suppress(Exception):
                (fn(timeout=float(join_timeout)) if name == "join" and join_timeout is not None else fn())
            return
    with contextlib.suppress(Exception): obj() if callable(obj) else None

def pool_tensor(
    tensor: torch.Tensor, *args: Any, dtype: torch.dtype, device: torch.device, cpu_pool: "TensorPagePool | None",
    dev_type: str | None = None, pinned_ok: bool | None = None,
) -> tuple[torch.Tensor, "TensorPagePool.Token | None", bool]:
    _ = args
    if not torch.is_tensor(tensor): raise TypeError(f"pool_tensor expects a torch.Tensor, got {type(tensor)}")
    
    if dev_type is None: dev_type = str(getattr(device, "type", "cpu"))
    if pinned_ok is None: pinned_ok = bool(is_pin_supported(str(dev_type)))

    if tensor.device.type != "cpu":
        if tensor.dtype != dtype:
            with contextlib.suppress(Exception): tensor = tensor.to(dtype=dtype, copy=False)
        return tensor, None, False

    with contextlib.suppress(Exception):
        if callable(is_pinned := getattr(tensor, "is_pinned", None)) and bool(is_pinned()) and tensor.dtype == dtype:
    ...
    if cpu_pool is not None and bool(pinned_ok):
        ...
        pinned = False
        with contextlib.suppress(Exception):
            if callable(is_pinned := getattr(buf, "is_pinned", None)): pinned = bool(is_pinned())
        return buf, token, pinned

    out = tensor.to(dtype=dtype, copy=False) if tensor.dtype != dtype else tensor
    pinned = False
    with contextlib.suppress(Exception):
        if callable(is_pinned := getattr(out, "is_pinned", None)): pinned = bool(is_pinned())
    return out, None, pinned

def stream_tensor(
    tensor: object, *args: Any, device: torch.device | str | int, cpu_pool: object,
    handle: "TensorPagePool.Token | None" = None, pinned: bool | None = None, dev_type: object | None = None,
    non_blocking_ok: object | None = None, backend: object | None = None, stream_fn: object | None = None,
    Event: object | None = None, fence_event_factory: object | None = None, can_stream_release: object | None = None,
) -> object:
    _ = args
    ...
    if dev_type is None: dev_type = str(getattr(device, "type", "cpu"))
    if non_blocking_ok is None: non_blocking_ok = bool(dev_type in ("cuda", "xpu"))
    pinned_ok = bool(is_pin_supported(str(dev_type)))

    if pinned is None:
        pinned = False
        with contextlib.suppress(Exception):
            if callable(is_pinned := getattr(tensor, "is_pinned", None)): pinned = bool(is_pinned())
    ...
    if tensor.device.type != "cpu" or (not bool(non_blocking_ok)):
        ...
        if handle is not None and cpu_pool is not None:
            with contextlib.suppress(Exception): cpu_pool.release(handle)
        return out

    if handle is None:
        return tensor.to(device, non_blocking=bool(non_blocking_ok and pinned and pinned_ok))

    if backend is None: backend = accelerator_type(str(dev_type))
    if stream_fn is None and backend is not None: stream_fn = getattr(backend, "current_stream", None)
    if Event is None and backend is not None: Event = getattr(backend, "Event", None)
    if can_stream_release is None: can_stream_release = bool(pinned and pinned_ok and callable(stream_fn) and (Event is not None))
    ...
    if callable(stream_fn):
        with contextlib.suppress(Exception):
            try: stream = stream_fn(device=device)
            except TypeError:
                try: stream = stream_fn(device)
                except TypeError: stream = stream_fn()

    try:
        if stream is not None:
            with accelerator_stream(stream, str(dev_type)):
                out = tensor.to(device, non_blocking=True)
            if callable(rec := getattr(tensor, "record_stream", None)):
                with contextlib.suppress(Exception): rec(stream)
        else:
            out = tensor.to(device, non_blocking=True)
        ...
        if cpu_pool is not None:
            try:
                evt = None
                if callable(fe := getattr(cpu_pool, "fence_event", None)) and fence_event_factory is not None:
                    with contextlib.suppress(Exception): evt = fe(handle, fence_event_factory)
                if evt is None:
                    if fence_event_factory is not None:
                        with contextlib.suppress(Exception): evt = fence_event_factory()
                    elif Event is not None:
                        with contextlib.suppress(Exception): evt = Event()
                if evt is not None:
                    if stream is not None:
                        try: evt.record(stream)
                        except TypeError: evt.record()
                    else: evt.record()
                    cpu_pool.release_after(handle, evt)
                else:
                    with contextlib.suppress(Exception): sync_accelerator(device)
                    with contextlib.suppress(Exception): cpu_pool.release(handle)
            except Exception:
                with contextlib.suppress(Exception): sync_accelerator(device)
                with contextlib.suppress(Exception): cpu_pool.release(handle)
...
def move_staged_pair_to_device(
    X_st: object, x_tok: "TensorPagePool.Token | None", x_pinned: bool,
    Y_st: object | None, y_tok: "TensorPagePool.Token | None", y_pinned: bool,
    ...
    Y_dev = to_device(Y_st, handle=y_tok, pinned=y_pinned) if Y_st is not None else None
    return X_dev, Y_dev, None, None
