...
# =============================================================================
from . import collate
from ..core.concurrency import Disposable, Mutex, is_free_threading_build, is_gil_enabled, new_affinity
...
_NODES_IMPORTED: bool = False
_NODES_LOCK = Mutex()
_device_mem_get_info = Memory.mem_get_info
_ENN_DIAG_DATALOADER_LOGGED: bool = False
...
def _require_nodes() -> None:
    ...
    with _NODES_LOCK:
        ...
        globals().update(
            {
                k: getattr(mod, k)
                for k in ["Governor", "Loader", "Mapper", "Multiplexer", "Sampler"]
            }
        )
...
def _is_source(obj: Any) -> bool:
    match obj:
        case dict() if "path" in obj and ("format" in obj or "kind" in obj):
            try:
                return bool(os.fspath(obj.get("path")))
            except Exception:
                return False
        case _:
            return False
...
def get_batch_length(loader: object) -> int:
    ...
    if hasattr(loader, "state_dict") and hasattr(loader, "load_state_dict"):
        ...
        if int(env_first_int(("ENN_LEN_PROBE_DEEPCOPY", "ENN_BATCHLEN_PROBE_DEEPCOPY"), default=1) or 0) > 0:
        ...
        max_batches = int(env_first_int(("ENN_LEN_PROBE_MAX_BATCHES", "ENN_BATCHLEN_PROBE_MAX_BATCHES"), default=0) or 0)
        max_seconds = float(env_first_float(("ENN_LEN_PROBE_MAX_SECONDS", "ENN_BATCHLEN_PROBE_MAX_SECONDS"), default=0.0) or 0.0)
        deadline_ns = int(time.perf_counter_ns() + (max_seconds * 1_000_000_000.0)) if max_seconds > 0.0 else 0
...
# =============================================================================
# Tensor Operations & Batch Sizing
# =============================================================================
def _stack_sequence(
    seq: Sequence[Any],
    *args: Any,
    ...
    for i, item in enumerate(seq[1:], 1):
        ...
        if ti.shape != t0.shape:
            raise ValueError(f"Dataset.preprocess: shape mismatch {t0.shape} vs {ti.shape}")
        out[i].copy_(ti)
    return out
...
def _get_random_batch(_sample_bytes: int, _device: torch.device, _N: int) -> Sequence[int]:
    if _sample_bytes <= 0 or _N <= 0:
        return [1]
    capB = 1024
    host_free = getattr(Memory, "available", lambda: None)()
    dev_free, _ = _device_mem_get_info(_device)
    effective_free = min(host_free, dev_free) if (host_free is not None and dev_free is not None) else (dev_free or host_free)
    
    if effective_free is not None:
        capB = max(1, int((max(0, int(effective_free)) * 0.80) // max(_sample_bytes * 4, 1)))
        
    capB = max(1, min(capB, int(_N)))
    cands = sorted({max(1, int(capB * f)) for f in (0.125, 0.25, 0.375, 0.5, 0.75, 1.0)})
...
def _h2d_counter(
    ...
    max_probe_bytes = int(env_first_int(("ENN_H2D_PROBE_MAX_BYTES", "ENN_H2D_PROBE_BYTES", "ENN_H2D_COUNTER_MAX_BYTES"), default=256 * 1024 * 1024) or 0)
    ...
    max_pin_bytes = max(0, int(env_first_int(("ENN_H2D_PROBE_MAX_PIN_BYTES", "ENN_H2D_PROBE_PIN_MAX_BYTES"), default=max_probe_bytes) or 0))
    ...
    for s in range(int(_steps) + int(_warmup)):
        ...
        if pin_batch and max_pin_bytes > 0:
            bbytes = int(xb.element_size() * xb.numel()) + (int(yb.element_size() * yb.numel()) if yb is not None else 0)
        ...
        if ev0 is not None and ev1 is not None:
            with accelerator(_device):
                ...
                if yb is not None: yb.to(_device, non_blocking=bool(pin_batch))
                ev1.record()
            _sync_device(_device)
...
def _set_batch_interval(
    ...
    *args: Any,
    ...
    if not isinstance(x_cpu, torch.Tensor): x_cpu = torch.as_tensor(x_cpu)
    if y_cpu is not None and not isinstance(y_cpu, torch.Tensor): y_cpu = torch.as_tensor(y_cpu)
        
    sbytes = sbytes_cached if sbytes_cached > 0 else _get_sample_size(x_cpu, y_cpu)
    if sbytes > 0 and sbytes_cached <= 0:
        with contextlib.suppress(Exception): setattr(_ds, "_S_sample_bytes", int(sbytes))
    if sbytes <= 0:
    ...
    if is_cuda and no_gil:
        _tmin_ms = env_first_float(("ENN_H2D_TMIN_MS_NO_GIL", "ENN_H2D_TMIN_NO_GIL"), default=max(1.0, float(_tmin_ms)))
        _tmax_ms = env_first_float(("ENN_H2D_TMAX_MS_NO_GIL", "ENN_H2D_TMAX_NO_GIL"), default=max(4.5, float(_tmax_ms)))
        
    B_cap = 1 << 16
    max_batch_env = int(env_first_int(("ENN_MAX_BATCH_SIZE", "ENN_MAX_BATCH"), default=0) or 0)
    per_sample = int(getattr(_ds, "_per_sample_mem_bytes", 0) or 0)
    per_sample_src = "ds._per_sample_mem_bytes" if per_sample > 0 else ""
    
    if per_sample <= 0:
        per_sample = int(env_first_int(("ENN_PER_SAMPLE_MEM_BYTES", "ENN_DEVICE_BYTES_PER_SAMPLE"), default=0) or 0)
        if per_sample > 0: per_sample_src = "env.ENN_PER_SAMPLE_MEM_BYTES"
    if per_sample <= 0:
        per_sample, per_sample_src = sbytes, "sample_bytes"
    ...
    budget_slack = max(1.0, min(4.0, float(env_first_float(("ENN_BUDGET_SLACK",), default=1.25) or 1.25)))

    def _get_bytes(name: str) -> int: return int(env_first_int((name,), default=0) or 0)
    def _get_opt_bytes(name: str) -> Optional[int]: return val if (val := _get_bytes(name)) > 0 else None
    ...
    tpl = BatchPolicy(
        ...
        min_batch=1,
        max_batch=B_cap,
        device_margin=float(env_first_float(("ENN_DEVICE_MARGIN",), default=0.90) or 0.90),
        host_margin=float(env_first_float(("ENN_HOST_MARGIN",), default=0.10) or 0.10),
        device_budget_ratio=float(env_first_float(("ENN_DEVICE_BUDGET_RATIO",), default=1.0) or 1.0),
        ...
        host_budget_ratio=float(env_first_float(("ENN_HOST_BUDGET_RATIO",), default=1.0) or 1.0),
    ...
    try:
        ...
        if host_avail > 0: host_free = host_avail
        with contextlib.suppress(Exception):
            if (_ht := Memory.total()) is not None and _ht > 0: host_total = int(_ht)
    except Exception:
        host_free = None
    ...
    if (tpl.device_budget_max_bytes is None or tpl.host_budget_max_bytes is None) and int(tpl.sample_bytes or 0) > 0:
        try:
            inflight = int(tpl.host_inflight_batches_per_proc())
            lw = max(1, int(getattr(tpl, "local_world_size", 1) or 1))
            target_ms = float(max(float(_tmin_ms), min(float(_tmax_ms), 0.5 * (float(_tmin_ms) + float(_tmax_ms)))))
            probe_bs = max(1, min(int(B_cap), 64))
            if int(max_batch_env) > 0: probe_bs = max(1, min(int(probe_bs), int(max_batch_env)))
            
            probe_max_bytes = int(env_first_int(("ENN_H2D_PROBE_MAX_BYTES", "ENN_H2D_PROBE_BYTES", "ENN_H2D_COUNTER_MAX_BYTES"), default=256 * 1024 * 1024) or 0)
            if probe_max_bytes > 0 and int(sbytes) > 0:
                probe_bs = max(1, min(int(probe_bs), int(probe_max_bytes) // max(1, int(sbytes))))
                
            med_probe = 0.0
            with contextlib.suppress(Exception):
                med_probe = float(_h2d_counter(x_cpu, y_cpu, _dev, probe_bs, _steps=4, _warmup=1))
                
            if isinstance(med_probe, (float, int)) and math.isfinite(float(med_probe)) and float(med_probe) > 0.0:
                ...
                target_batch_samples = max(1, min(int(B_cap), int((64 * 1024 * 1024) // max(1, int(tpl.sample_bytes)))))
            ...
            if new_dev_cap is None:
                cap_dev = int(float(int(tpl.sample_bytes) * int(target_batch_samples)) * float(budget_slack))
                if dev_total is not None and int(dev_total) > 0: cap_dev = min(int(cap_dev), int(dev_total))
                new_dev_cap = None if cap_dev <= 0 else max(0, int(cap_dev))
                
            if new_host_cap is None and int(tpl.host_sample_bytes or 0) > 0:
                cap_host = int(float(int(tpl.host_sample_bytes) * max(1, inflight) * max(1, lw) * int(target_batch_samples)) * float(budget_slack))
                if host_total is not None and int(host_total) > 0: cap_host = min(int(cap_host), int(host_total))
                new_host_cap = None if cap_host <= 0 else max(0, int(cap_host))
                
            if (new_dev_cap != tpl.device_budget_max_bytes) or (new_host_cap != tpl.host_budget_max_bytes):
                tpl = replace(tpl, device_budget_max_bytes=new_dev_cap, host_budget_max_bytes=new_host_cap)
        except Exception:
            pass
            
    cap_from_mem = tpl.suggest_batch(dev_free=dev_free, host_free=host_free, dev_total=dev_total, host_total=host_total, local_world_size=_lws)
    if cap_from_mem > 0: B_cap = min(B_cap, cap_from_mem)
    B_cap = max(1, min(int(B_cap), len(_ds)))
    if int(max_batch_env) > 0: B_cap = max(1, min(B_cap, int(max_batch_env)))
    with contextlib.suppress(Exception): setattr(_ds, "_S_B_cap", int(B_cap))
        
    candidates = _get_random_batch(sbytes, _dev, len(_ds))
    B = min(candidates[-1], B_cap) if candidates else min(64, B_cap)
    
    if b_init_hint is not None:
        try:
            B_hint = max(1, min(int(B_cap), int(b_init_hint)))
            if candidates:
                cands = [int(c) for c in candidates if isinstance(c, int) and c > 0 and int(c) <= int(B_cap)]
                if cands:
                    le = [c for c in cands if int(c) <= int(B_hint)]
                    B = int(max(le)) if le else int(min(cands))
                else: B = int(B_hint)
            else: B = int(B_hint)
    ...
    min_batch_env = int(env_first_int(("ENN_MIN_BATCH_SIZE", "ENN_MIN_BATCH"), default=0) or 0)
    if min_batch_env <= 0 and is_cuda and no_gil:
        min_batch_env = int(env_first_int(("ENN_MIN_BATCH_SIZE_NO_GIL", "ENN_MIN_BATCH_NO_GIL"), default=8) or 0)
        
    global _ENN_DIAG_DATALOADER_LOGGED
    diag = int(env_first_int(("ENN_DIAG_BATCH_SIZES", "ENN_DIAG_BATCHING", "ENN_DIAG_BATCH"), default=0) or 0)
    ...
    *args: Any,
    ...
    try:
        ...
        for ds in datasets.values():
            ...
            if not isinstance(x, torch.Tensor): x = torch.as_tensor(x)
            if y is not None and not isinstance(y, torch.Tensor): y = torch.as_tensor(y)
            sb = _get_sample_size(x, y)
            if sb > 0:
                with contextlib.suppress(Exception): setattr(ds, "_S_sample_bytes", int(sb))
            sbytes_max = max(sbytes_max, sb)
            
        if sbytes_max <= 0: return int(pf)
        bytes_per_batch = int(sbytes_max) * int(bs)
        if bytes_per_batch <= 0: return int(pf)
        
        pf_cap = max(1, int(budget // max(1, bytes_per_batch)))
        with contextlib.suppress(Exception):
            hard = int(loader_policy.hard_inflight_batches(device))
            soft_cap = max(1, int(hard * max(1, int(loader_policy.soft_cap_multiplier))))
...
def _fetch_iterate_sample(
    ...
    *args: Any,
    ...
    match sample:
        case list() | tuple() if sample and all(isinstance(elem, tuple) and len(elem) == 2 for elem in sample):
            batches: list[Any] = []
            for k, span in sample:
                if (ds := datasets.get(str(k))) is None: continue
                s, e = int(span[0]), int(span[1])
                batch = ds.get(s, e)
                if batch is not None:
                    batches.append(batch)
            if not batches: return None
            if len(batches) == 1: return collate_fn(batches[0])
            return collate_fn(_fetch_merge_batches(batches))
        case tuple() if len(sample) == 2:
            k, span = sample
            if (ds := datasets.get(str(k))) is None: return None
            s, e = int(span[0]), int(span[1])
            batch = ds.get(s, e)
            return collate_fn(batch) if batch is not None else None
        case _:
...
def _fetch_merge_batches(batches: Sequence[Any]) -> Any:
    ...
    if all(isinstance(b, Mapping) for b in batches):
        ...
        for key in keys:
            ...
            if key == "row_ids":
                tensors = [v if isinstance(v, torch.Tensor) else torch.as_tensor(v) for v in vals if v is not None]
                if tensors: merged[key] = torch.cat(tensors, dim=0)
                continue
                
            if all(isinstance(v, (list, tuple)) for v in vals):
                merged[key] = list(chain.from_iterable(v if isinstance(v, list) else list(v) for v in vals))
                continue
            ...
            merged[key] = list(chain.from_iterable(v if isinstance(v, (list, tuple)) else [v] for v in vals))
        return merged
        
    return list(chain.from_iterable(b if isinstance(b, (list, tuple)) else [b] for b in batches))


def _fetch_normalize_sources(sources: Any) -> Dict[str, Source]:
    match sources:
        case Mapping() if not _is_source(sources):
            out: Dict[str, Source] = {}
...
def _fetch_build_sampler_nodes(
    ...
    *args: Any,
    ...
    for k, ds in datasets.items():
        sn = ds.compose(batch_size=int(bs), shuffle=bool(shuffle), seed=int(seed), key=str(k))
        if len(ds) > 0:
            nodes[str(k)] = sn
...
def iter_dataset(data: object) -> tuple[list[tuple[str, object]], object | None]:
    match data:
        case TensorDictBase():
            return ([("0", data)], None)
        case Mapping() if data and all(isinstance(v, (TensorDictBase, collections.abc.Mapping)) for v in data.values()):
        ...
        case Sequence() if data and not isinstance(data, (str, bytes, bytearray)) and all(isinstance(d, (TensorDictBase, collections.abc.Mapping)) for d in data):
...
def new_dataset(
    ...
    *args: Any,
    split: str = "train",
    val_frac: float = 0.0,
    sampler_scale: Optional[Any] = None,
    **kwargs: Any,
) -> Any:
    _require_nodes()
    ...
    fmt = str(source.get("format") or source.get("kind") or "")
    if not fmt:
    ...
    import enn_torch.data.nodes as nodes
    return nodes.Sampler(path, split=sp, val_frac=vf, sampler_scale=sampler_scale)
...
def compose(
    ...
    **kwargs: Any,
    ...
    with contextlib.suppress(Exception): new_affinity(io_workers=io_workers)
        
    sampler = nodes.Multiplexer(stop_criteria="ALL_DATASETS_EXHAUSTED", seed=int(seed), weights=weights)
    source = sampler.compose(node_or_nodes)
    if epochables is not None and getattr(sampler, "_node", None) is not None:
        with contextlib.suppress(Exception): epochables.insert(0, sampler)
...
def fetch(
    ...
    *args: Any,
    ...
    wp = worker_policy if isinstance(worker_policy, WorkerPolicy) else WorkerPolicy.optimize(device=device_obj)
    ...
    allocated = Disposable()
    scale_ctl = sampler_scale if sampler_scale is not None else nodes.Governor()
    ...
    def _create_loader_stage(
        ...
        datasets = _fetch_build_datasets(
            specs, split=split, val_frac=float(val_frac), sampler_scale=scale_ctl,
            allocated=allocated, collect_epochables=collect_epochables, epochables=epochables,
        )
        ...
        if bs <= 0:
            bs = _fetch_auto_batch_size(
                datasets, device_obj, pf=pf_depth_fixed, io_workers=io_workers, prebatch=prebatch, worker_policy=wp, fallback=1,
            )
            
        pf = _fetch_cap_pf_depth(
            datasets, device_obj, pf=pf_depth_fixed, bs=bs, loader_policy=lp, io_workers=io_workers, prebatch=prebatch,
        )
        pf = int(max(1, min(pf, pf_depth_fixed, 8)))
        
        if not (batch_size and int(batch_size) > 0) and pf != pf_depth_fixed:
            bs = _fetch_auto_batch_size(
                datasets, device_obj, pf=pf, io_workers=io_workers, prebatch=prebatch, worker_policy=wp, fallback=bs,
            )

        n_nodes, lengths = _fetch_build_sampler_nodes(datasets, bs=bs, shuffle=shuffle, seed=seed)
        if not n_nodes: raise RuntimeError(f"No non-empty sources for split={split}")

        weights_out = None
        if len(n_nodes) > 1 and weights:
            w_map = {str(k): float(v) for k, v in dict(weights).items() if str(k) in n_nodes} if isinstance(weights, Mapping) else None
            if w_map:
                if not any(v > 0.0 for v in w_map.values()): raise ValueError("Weights must be > 0")
                weights_out = w_map

        iter_fn = partial(_fetch_iterate_sample, datasets=datasets, collate_fn=collate_fn)
        _, mapped, _ = compose(
            n_nodes, device=device_obj, map_fn=iter_fn, prefetch_factor=pf, non_blocking_copy=non_blocking_copy,
            io_workers=io_workers, prebatch=prebatch, weights=weights_out, epochables=epochables if collect_epochables else None, seed=seed,
        )
        return nodes.Loader.compose(
            mapped, device=device_obj, prefetch_factor=pf, non_blocking=non_blocking_copy, length=sum(lengths.values()) if lengths else None,
        )

    train_loader = _create_loader_stage("train", train_shuffle, train_weights, collect_epochables=True, epochables=train_epochables)
    val_epochables: list[Any] = []
    val_loader = _create_loader_stage("val", False, val_weights, collect_epochables=True, epochables=val_epochables) if float(val_frac) > 0.0 else None
...
def preload_memmap(
    ...
    *args: Any,
    ...
    count = _len0(raw_X)
    if count <= 0: raise ValueError("cannot create memmap with zero samples")

    if not bool(features_only):
        if raw_Y is None:
            if not bool(allow_missing_labels): raise ValueError("preload_memmap expects 'labels' unless allow_missing_labels=True")
        else:
            if _len0(raw_Y) != int(count): raise ValueError("features and labels must have the same length")
    ...
    get_by_indices = _BatchIndexGetter(raw_X, raw_Y, features_only=bool(features_only)) if bool(shuffle) else None
    ...
    collate.stream_memmap(
        ...
        default_label_shape=tuple(int(x) for x in default_label_shape) if default_label_shape is not None else None,
...
class Session:
    ...
    def open(
        ...
        *args: Any,
        ...
        if self.sampler_scale is None:
            self.sampler_scale = nodes.Governor()
        ...
        dl = fetch(
            sources=self.sources, device=dev, val_frac=float(self.val_frac),
            non_blocking_copy=bool(self.non_blocking_copy), labels_dtype=self.labels_dtype,
            sanitize=bool(self.sanitize), flatten_features=bool(self.flatten_features),
            train_shuffle=bool(self.train_shuffle), seed=int(self.seed),
            train_weights=self.train_weights, val_weights=self.val_weights,
            worker_policy=wp, sampler_scale=self.sampler_scale, loader_policy=self.loader_policy,
        )
        ...
        if train_state and train_loader is not None and hasattr(train_loader, "load_state_dict"):
            with contextlib.suppress(Exception): train_loader.load_state_dict(train_state)
        if val_state and val_loader is not None and hasattr(val_loader, "load_state_dict"):
            with contextlib.suppress(Exception): val_loader.load_state_dict(val_state)
            
        self.training_loader = self.loader_policy.wrap_input(train_loader, dev, name="train-input") if train_loader is not None else None
        self.validation_loader = self.loader_policy.wrap_input(val_loader, dev, name="val-input") if val_loader is not None else None
        
        with contextlib.suppress(Exception):
            if self.training_loader is not None: setattr(self.training_loader, "_enn_sampler_scale", self.sampler_scale)
            if self.validation_loader is not None: setattr(self.validation_loader, "_enn_sampler_scale", self.sampler_scale)
        with contextlib.suppress(Exception):
            if self.raw_training_loader is not None and self.training_loader is not None:
                if (epochables := getattr(self.raw_training_loader, "_enn_epochables", None)) is not None:
    ...
    def close(self: Self) -> None:
        ...
        if (keep := getattr(self, "disposable", None)) is not None:
            with contextlib.suppress(Exception): keep.cleanup()
        self._opened = False
...
class Dataset(Generic[TExtra]):
    ...
    def _parse_dtypes_env(value: str) -> Tuple[torch.dtype, ...]:
        ...
        for token in str(value).split(","):
            ...
            if isinstance(dtype, torch.dtype): entries.append(dtype)
        return tuple(entries)

    def _refresh_dtypes_from_env(self: Self) -> None:
        if float_env := env_first(("ENN_DATA_FLOAT_DTYPES", "ENN_FLOAT_DTYPES")):
            if parsed := self._parse_dtypes_env(float_env): self.float_dtypes = parsed
        if int_env := env_first(("ENN_DATA_INT_DTYPES", "ENN_INT_DTYPES")):
            if parsed := self._parse_dtypes_env(int_env): self.int_dtypes = parsed

    def _refresh_quant_from_env(self: Self) -> None:
        if (bits := env_first_int(("ENN_DATA_INT_QUANT_BITS", "ENN_INT_QUANT_BITS"), default=0)) > 0:
            self.int_quant_bits = int(bits)

    @property
    def device_stats(self: Self) -> "Any":
    ...
    def refresh(self: Self) -> "Dataset[TExtra]":
        ...
        if not self.float_dtypes: self.float_dtypes = tuple(getattr(dev_stats, "float_dtypes", ()))
        if not self.int_dtypes: self.int_dtypes = tuple(getattr(dev_stats, "int_dtypes", ()))
        if self.int_quant_bits is None: self.int_quant_bits = int(getattr(dev_stats, "int_quant_bits", 8))
        
        if not self.float_dtypes:
            floats: list[torch.dtype] = [torch.float32]
            match self.device_type:
                case "cuda" if is_accelerator_available("cuda"):
                    floats.insert(0, torch.float16)
                    if self.is_cuda_bf16_supported(self.device): floats.insert(0, torch.bfloat16)
                case "cpu" if self.is_cpu_bf16_supported():
    ...
    def preprocess(
        ...
        *args: Any,
        ...
        cast_to_dtype: bool = True,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Sequence[Any], Tuple[int, ...]]:
        features: Optional[torch.Tensor] = None
        labels: Optional[torch.Tensor] = None
        keys: Sequence[Any] = ()
        feat_dtype = self.feature_dtype if bool(cast_to_dtype) else None
        label_dtype = self.label_float_dtype if bool(cast_to_dtype) else None
        ...
        match data:
            case TensorDictBase():
                ...
                if bool(return_keys):
                    row_ids = data.get("row_ids", data.get("keys", None))
                    keys = row_ids if row_ids is not None else ()
                    
            case Mapping() if collate._resolve_key(data, collate._FEATURE_KEY_ALIASES, "feature", False) is not None or collate._resolve_key(data, collate._LABEL_KEY_ALIASES, "label", False) is not None:
                fkey = collate.get_feature_key(data)
                features = data.get(fkey, None)
                lkey = collate.get_label_key(data, required=False)
                labels = data.get(lkey, None) if lkey is not None else None
            ...
            case Mapping():
                ...
                try:
                    ...
                    match v0:
                        case tuple() | list() if len(v0) >= 2:
                            n = len(data)
                            keys_list: Optional[list[Any]] = [k0] if bool(return_keys) else None
                            x0 = collate._to_safe_tensor(v0[0], feat_dtype)
                            if x0 is None: raise ValueError("Dataset.preprocess: missing feature in tuple mapping")
                            ...
                            for i, (k, v) in enumerate(it, start=1):
                                if keys_list is not None: keys_list.append(k)
                                if not isinstance(v, (list, tuple)) or len(v) < 1: raise ValueError("Dataset.preprocess: invalid tuple mapping element")
                                x_i = collate._to_safe_tensor(v[0], feat_dtype)
                                if x_i is None: raise ValueError("Dataset.preprocess: missing feature in tuple mapping")
                                if tuple(x_i.shape) != tuple(x0.shape): raise ValueError(f"Dataset.preprocess: inconsistent feature shapes in tuple mapping: {tuple(x0.shape)} vs {tuple(x_i.shape)}")
                                feat[i].copy_(x_i)
                                if labels_out is not None:
                                    if len(v) < 2 or v[1] is None: labels_out = None
                                    else:
                                        y_i = collate._to_safe_tensor(v[1], label_dtype)
                                        if y_i is None or y0 is None or tuple(y_i.shape) != tuple(y0.shape): labels_out = None
                                        else: labels_out[i].copy_(y_i)
                            features = feat
                            labels = labels_out
                            if bool(return_keys) and keys_list is not None: keys = keys_list
                        ...
                        case _:
                            ...
                            if bool(return_keys): keys = keys_list
                            parsed = False
                            if key_as_feature and keys_list and values_list:
                                has_missing_labels = any((v is None for v in values_list))
                                try:
                                    features = _stack_sequence(keys_list, dtype=feat_dtype, reshape_1d=True)
        ...
        if features.ndim == 0: features = features.reshape(1, 1)
        elif features.ndim == 1: features = features.reshape(1, -1)
        ...
        if labels is not None:
            ...
            if labels.ndim == 0: labels = labels.reshape(1, 1)
            if labels.shape[0] != features.shape[0]: labels = labels.reshape(features.shape[0], -1)
            if not bool(labels.is_contiguous()) and not _is_lazy_tensor(labels): labels = labels.contiguous()
        ...
        if bool(return_keys):
            match keys:
                case torch.Tensor():
                    with contextlib.suppress(Exception): keys = keys.detach().cpu().tolist()
                case None:
                    keys = ()
            if not keys: keys = range(int(features.shape[0]))
        else:
            keys = ()
    ...
    def tensor_scale_stats(t: torch.Tensor) -> Dict[str, Any]:
        ...
        if t.numel() == 0:
            return {
                "has_scale": False, "has_nonfinite": False,
                "scale_max_abs": None, "scale_min_value": None, "scale_max_value": None,
                "scale_min_positive": None, "scale_is_integral": None,
            }
            
        is_complex = False
        _is_complex_fn = getattr(torch, "is_complex", None)
        if callable(_is_complex_fn):
            with contextlib.suppress(Exception): is_complex = bool(_is_complex_fn(t))
        else:
            v = getattr(t, "is_complex", False)
            with contextlib.suppress(Exception): is_complex = bool(v() if callable(v) else v)
        ...
        if t.is_floating_point():
            ...
            if all_finite:
                ...
                if min_val is None or max_val is None:
                    ...
                    with contextlib.suppress(Exception):
                        pos = x > 0
                        if bool(pos.any().item()): min_pos = float(x[pos].min().item())
                    with contextlib.suppress(Exception):
                        neg = x < 0
                        if bool(neg.any().item()):
                            max_neg = float(x[neg].max().item())
                            cand = float(-max_neg)
                            if cand > 0.0: min_pos = cand if (min_pos is None or cand < min_pos) else min_pos
                return {
                    "has_scale": True, "has_nonfinite": bool(has_nonfinite),
                    "scale_max_abs": max_abs, "scale_min_value": min_val, "scale_max_value": max_val,
                    "scale_min_positive": min_pos, "scale_is_integral": None,
                }
            ...
            if bool(finite.any().item()):
                ...
                max_abs = float(max(abs(min_val), abs(max_val))) if min_val is not None and max_val is not None else float("nan")
                min_pos = None
                with contextlib.suppress(Exception):
                    pos = xf > 0
                    if bool(pos.any().item()): min_pos = float(xf[pos].min().item())
                with contextlib.suppress(Exception):
                    neg = xf < 0
                    if bool(neg.any().item()):
        ...
        return {
            "has_scale": True, "has_nonfinite": False,
            "scale_max_abs": max_abs, "scale_min_value": min_i, "scale_max_value": max_i,
            "scale_min_positive": None, "scale_is_integral": True,
        }
    ...
    def merge_scale_stats(a: Mapping[str, Any], b: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        out["has_nonfinite"] = bool(a.get("has_nonfinite")) or bool(b.get("has_nonfinite"))
        out["scale_max_abs"] = _merge_opt(a.get("scale_max_abs"), b.get("scale_max_abs"), lambda v1, v2: v1 if v1 >= v2 else v2)
        out["scale_min_value"] = _merge_opt(a.get("scale_min_value"), b.get("scale_min_value"), lambda v1, v2: v1 if v1 <= v2 else v2)
        out["scale_max_value"] = _merge_opt(a.get("scale_max_value"), b.get("scale_max_value"), lambda v1, v2: v1 if v1 >= v2 else v2)
        out["scale_min_positive"] = _merge_opt(a.get("scale_min_positive"), b.get("scale_min_positive"), lambda v1, v2: v1 if v1 <= v2 else v2)
        
        ia, ib = a.get("scale_is_integral"), b.get("scale_is_integral")
        if ia is None: out["scale_is_integral"] = ib
        elif ib is None: out["scale_is_integral"] = ia
        else: out["scale_is_integral"] = bool(ia) and bool(ib)
        return out
    ...
    def is_fp32_castable(
        ...
        *args: Any,
        ...
        if not stats.get("has_scale"): return True
        if bool(stats.get("has_nonfinite")): return False
        
        max_abs = stats.get("scale_max_abs")
        if max_abs is None: return True
        
        try: max_abs_f = float(abs(max_abs))
        except Exception: return False
        
        if not math.isfinite(max_abs_f): return False
        
        info = torch.finfo(torch.float32)
        if max_abs_f > float(info.max) / max(1.0, float(safety_margin)): return False
        ...
        if action == "forbid":
            ...
            if min_pos is not None:
                try: min_pos_f = float(min_pos)
                except Exception: return False
                if math.isfinite(min_pos_f) and min_pos_f > 0.0:
                    if min_pos_f < float(info.tiny) * max(1.0, float(safety_margin)): return False
        return True
    ...
    def update_scale_stats(self: Self, stats: Mapping[str, Any]) -> None:
        ...
        self.is_negotiable = bool(
            self.has_scale and (not self.has_nonfinite) and
            self.is_fp32_castable(stats, underflow_action=self.underflow_action, safety_margin=1.0)
        )
    ...
    @classmethod
    def is_float8_supported(cls: type[Self], device: Optional[Union[torch.device, str]] = None) -> Tuple[bool, str]:
        return is_float8_supported(device)

    @classmethod
    def is_int8_supported(cls: type[Self], device: Optional[Union[torch.device, str]] = None) -> Tuple[bool, str]:
        return is_int8_supported(device)

    @classmethod
    def is_int4_supported(cls: type[Self], device: Optional[Union[torch.device, str]] = None) -> Tuple[bool, str]:
