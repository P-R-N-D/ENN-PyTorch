...
_FEATURE_KEY_ALIASES: frozenset[str] = frozenset(
    {"x", "feature", "features", "input", "inputs", "in"}
)
_LABEL_KEY_ALIASES: frozenset[str] = frozenset(
    {"y", "label", "labels", "output", "outputs", "out"}
)


# =============================================================================
# Internal TensorDict & Mapping Helpers
# =============================================================================
...
def _resolve_key(
    data: Any, aliases: frozenset[str], name: str, required: bool
) -> Optional[str]:
    match data:
        case Mapping() | TensorDictBase():
            matches = [
            ...
            if required:
                raise KeyError(f"Expected exactly one {name} key among {sorted(aliases)}; found {matches or 'none'}")
            return None
        case _:
...
def get_feature_key(data: Any) -> str:
    res = _resolve_key(data, _FEATURE_KEY_ALIASES, "feature", True)
    if res is None:
        raise KeyError("Feature key is strictly required but not found.")
    return res
...
# =============================================================================
# Value Coercion & Extraction Helpers
# =============================================================================
    ...
    match meta:
        case Mapping():
            ...
            if isinstance(ss, Mapping):
                return bool(ss.get("has_scale") or any(ss.get(k) is not None for k in keys))
            return False
        case _:
            return False
...
def _to_safe_tensor(obj: Any, dtype: Optional[torch.dtype] = None) -> Optional[torch.Tensor]:
    ...
    if dtype is None:
        return t
    return t.to(dtype=dtype, copy=False) if t.dtype != dtype else t


def _td_batch_size_from_X(x: Any) -> List[int]:
    match x:
        case torch.Tensor() if x.ndim >= 1:
            return [int(x.shape[0])]
        case _:
            return []


def _coerce_path(path: PathLike | None) -> Optional[str]:
    if path is None:
        return None
...
def _coerce_prediction_output(output: object) -> str:
    match str(output).strip().lower():
        case "file" | "disk" | "lazy" | "h5" | "hdf5":
            return "file"
        case _:
            return "memory"
...
def _coerce_prediction_overwrite(overwrite: object) -> str:
    match str(overwrite).strip().lower():
        case "resume":
            return "resume"
        case "replace" | "overwrite" | "force":
            return "replace"
        case "ignore" | "skip":
            return "ignore"
        case _:
...
def _get_prediction_dtype(chunks_dir: PathLike, *args: Any, default: torch.dtype = torch.float32) -> torch.dtype:
    del args
...
def _expand_multinode_sources(spec: Any) -> Tuple[Any, bool]:
    match spec:
        case dict() if "path" in spec and "kind" in spec:
            ...
            match payload:
                case dict():
                    return {str(k): {"kind": "memmap", "path": os.path.join(root, str(v))} for k, v in payload.items()}, True
                case list():
...
def _preload_slice_any(obj: object | None, s: int, e: int, *args: object, name: str) -> object | None:
    del args, name
    match obj:
        case None:
            return None
        case _ if hasattr(obj, "__getitem__"):
            try:
                return obj[s:e]
            except Exception:
...
def _primary_device(device_spec: Union[torch.device, List[torch.device]]) -> torch.device:
    match device_spec:
...
def _idx_to_cpu_int64(idx: Any) -> torch.Tensor:
    ...
    if idx_t.dtype not in (torch.int64, torch.int32):
        idx_t = idx_t.to(dtype=torch.int64, copy=False)
    idx_t = idx_t.reshape(-1)
    return idx_t.to(dtype=torch.int64, copy=False)


def _validate_row_contiguity(rows: torch.Tensor) -> Tuple[bool, int, int]:
    rows_view = rows.reshape(-1)
    n = int(rows_view.numel())
    if n <= 0:
        return (True, 0, 0)
    start = int(rows_view[0].item())
    if n == 1:
        return (True, start, start + 1)
    last = int(rows_view[-1].item())
    if last != start + n - 1:
        return (False, 0, 0)
    if not bool(torch.all(rows_view[1:] == rows_view[:-1] + 1)):
        return (False, 0, 0)
...
def _to_numpy_dtype(dtype: torch.dtype) -> type[numpy.generic]:
    ...
    match dtype:
        case torch.float16: return numpy.float16
        case torch.float32: return numpy.float32
        case torch.float64: return numpy.float64
        case torch.bfloat16: return np_bfloat16
        case torch.int8: return numpy.int8
        case torch.uint8: return numpy.uint8
        case torch.int16: return numpy.int16
        case torch.int32: return numpy.int32
        case torch.int64: return numpy.int64
        case torch.bool: return numpy.bool_
        case _: return numpy.float32
...
def _atomic_h5_op(
    ...
    fd, tmp = tempfile.mkstemp(prefix=os.path.basename(out_path) + ".", suffix=".tmp", dir=os.path.dirname(out_path))
    os.close(fd)
...
def stream_memmap(
    *args: Any,
    ...
    env_chunk = env_first_int(("ENN_MEMMAP_CHUNK_SIZE", "ENN_MEMMAP_CHUNK"), 0)
    if int(env_chunk) > 0:
        chunk_size = int(env_chunk)
        
    req_chunk = int(chunk_size or 0)
    auto_chunk = req_chunk <= 0
    chunk_first = 1 if auto_chunk else max(1, min(count_i, req_chunk))
    ...
    while int(s_i) < int(count_i):
        ...
        if int(n) != int(expected):
            raise RuntimeError(f"Pass1 batch size mismatch for out_dir={out_dir!r}: expected {expected}, got {n} (s={s_i}, e={e_i}).")
        ...
        if not bool(auto_chunk_adjusted):
            try:
                target_bytes = int(env_first_int(("ENN_MEMMAP_CHUNK_BYTES",), 0))
                if target_bytes <= 0:
                    target_mb = int(env_first_int(("ENN_MEMMAP_CHUNK_MB",), 64))
                    target_bytes = int(target_mb) * 1024 * 1024
                avail = int(Memory.available() or 0)
                if avail > 0:
                    target_bytes = int(min(int(target_bytes), max(8 * 1024 * 1024, int(avail) // 16)))
                ...
                new_chunk = int(max(1, min(int(count_i), int(target_bytes) // int(row_bytes))))
                pass1_max = int(env_first_int(("ENN_MEMMAP_PASS1_CHUNK_MAX",), 256))
                if pass1_max > 0:
        ...
        if bool(features_only):
            ...
            if lb_flat is None:
                l_stats = {
                    ...
                    "scale_max_abs": 0.0, "scale_min_value": 0.0, "scale_max_value": 0.0,
    ...
    if auto_chunk:
        ...
        try:
            ...
            pass
        chunk_second = int(max(1, min(count_i, int(target_bytes) // int(row_bytes))))
    else:
    ...
    features_mmt = MemoryMappedTensor.empty((count_i, int(in_dim)), dtype=store_float, filename=features_path, existsok=True)
    
    write_labels = not bool(features_only)
    labels_mmt = None
    if write_labels:
        labels_mmt = MemoryMappedTensor.empty((count_i, *tuple(label_shape)), dtype=store_float, filename=labels_path, existsok=True)
    ...
    if bool(shuffle):
        ...
        max_elems = env_first_int(("ENN_MEMMAP_RANDPERM_MAX_ELEMS", "ENN_MEMMAP_SHUFFLE_MAX_ELEMS"), 5_000_000)
    ...
    for s in range(0, count_i, int(chunk_second)):
        ...
        if n != expected:
            raise RuntimeError(f"Pass2 batch size mismatch for out_dir={out_dir!r}: expected {expected}, got {n} (s={s}, e={e}).")
...
def iter_source_path(obj: Any) -> Iterator[str]:
    match obj:
        case None:
            return
        case str():
...
def merge_meta_info(metas: Any) -> Dict[str, Any]:
    ...
    match metas:
        case None:
            return {}
        case list() as seq if seq and isinstance(seq[0], dict):
            return _merge_dicts(seq)
        case _:
...
def load_scaler_stats(sources: Any) -> Optional[Dict[str, Any]]:
    ...
    for path in iter_source_path(expanded):
        ...
        def _coerce_bounds(v: Any, ref_shape: tuple[int, ...]) -> Optional[torch.Tensor]:
            if v is None:
                return None
            t = v.detach().to(dtype=torch.float64, device="cpu") if isinstance(v, torch.Tensor) else torch.as_tensor(v, dtype=torch.float64)
        ...
        local_have_bounds = (local_xmin is not None and local_xmax is not None and local_ymin is not None and local_ymax is not None)
    ...
    if have_bounds and x_min is not None and x_max is not None and y_min is not None and y_max is not None:
        out.update({"x_min": x_min, "x_max": x_max, "y_min": y_min, "y_max": y_max})
    if have_qbounds and y_q_low is not None and y_q_high is not None:
        out.update({"y_q_low": y_q_low, "y_q_high": y_q_high})
    return out
...
def warmup_scaler_stats(model: Any, train_loader: Any, ops_or_sources: Any) -> None:
    ...
    if stats:
        ...
        if x_min is not None: x_min = x_min.to(dev_x)
        if x_max is not None: x_max = x_max.to(dev_x)
        if y_min is not None: y_min = y_min.to(dev_y)
        if y_max is not None: y_max = y_max.to(dev_y)
    else:
        x_cnt = 0
        y_cnt = 0
        x_sum = None
...
def expand_source(sources: Any) -> Any:
    ...
    if ok: return expanded
    match sources:
        case list() | tuple() as seq if len(seq) == 1:
            expanded, ok = _expand_multinode_sources(seq[0])
            if ok: return expanded
    return sources
    ...
    *args: Any,
    ...
    with h5py.File(p, "r") as f:
        ...
        if x_kind in ("O", "S", "U", "V") or y_kind in ("O", "S", "U", "V"):
            raise ValueError(f"predictions file has unsupported dtypes: X.dtype={getattr(dX, 'dtype', None)!r}, Y.dtype={getattr(dY, 'dtype', None)!r} ({p!r})")
        ...
        if in_dim is not None and int(x_shape[1]) != int(in_dim):
            raise ValueError(f"predictions file has unexpected X feature_dim: got {int(x_shape[1])}, expected {int(in_dim)} ({p!r})")
        ...
        if out_shape is None:
            ...
            if len(y_shape) != 1 + len(out_shape_t):
                raise ValueError(f"predictions file has unexpected Y rank: got {len(y_shape)}, expected {1 + len(out_shape_t)} ({p!r})")
            if tuple(int(d) for d in y_shape[1:]) != out_shape_t:
                raise ValueError(f"predictions file has unexpected Y shape: got {tuple(int(d) for d in y_shape[1:])}, expected {out_shape_t} ({p!r})")
    return int(n)
...
def concat_memory_mapped_tensor(
    ...
    *args: Any,
    ...
    y_out = MemoryMappedTensor.empty(full_shape, dtype=store_float, filename=os.fspath(out_path), existsok=True)
    manifest_path = os.path.join(os.fspath(chunks_dir), "manifest.json")
    manifest = read_json(manifest_path)
    if not isinstance(manifest, dict):
        raise ValueError(f"Invalid manifest: {manifest_path}")
    if bool(manifest.get("variable_shape", False)):
        raise NotImplementedError("Variable-shaped predictions cannot be assembled into a single dense MemoryMappedTensor.")
...
def concat_segment_h5(
    ...
    *args: Any,
    ...
    step = int(chunk_size)
    h5_kwargs = _h5_filter_kwargs(h5_compression, h5_compression_opts, h5_shuffle)
    ...
    with h5py.File(out_path, "w") as f:
        ...
        if use_filters:
            dset_x_kwargs["chunks"] = (min(int(count), step), *[int(x) for x in x_mmt.shape[1:]])
            dset_y_kwargs["chunks"] = (min(int(count), step), *out_shape_t)
        dset_X = f.create_dataset(
            "X", shape=tuple(x_mmt.shape), dtype=_to_numpy_dtype(x_mmt.dtype), **dset_x_kwargs
        )
        dset_Y = f.create_dataset(
            "Y", shape=(int(count), *out_shape_t), dtype=np_float, **dset_y_kwargs
        )
        ...
        if bool(manifest.get("variable_shape", False)):
            raise NotImplementedError("Variable-shaped predictions cannot be stored as a dense HDF5 dataset.")
...
def write_predictions_h5_from_memmap(
    ...
    *args: Any,
    ...
    with h5py.File(out_path, "w") as f:
        dset_X = f.create_dataset(
            "X", shape=(n, int(x_mmt.shape[1])), dtype=x_np_dtype, chunks=(min(n, step), int(x_mmt.shape[1])), **h5_kwargs
        )
        dset_Y = f.create_dataset(
            "Y", shape=(n, *[int(x) for x in y_mmt.shape[1:]]), dtype=y_np_dtype, chunks=(min(n, step), *[int(x) for x in y_mmt.shape[1:]]), **h5_kwargs
        )
        for s in range(0, n, step):
            e = min(n, s + step)
    ...
    *args: Any,
    ...
    return _atomic_h5_op(
        ...
        lambda tmp: write_predictions_h5_from_memmap(
            tmp, memmap_dir=memmap_dir, pred_path=pred_path, chunk_size=chunk_size,
            h5_compression=h5_compression, h5_compression_opts=h5_compression_opts, h5_shuffle=h5_shuffle,
        ),
    )
...
def copy_predictions_h5_atomic(
    ...
    *args: Any,
    ...
    res = _atomic_h5_op(dst_path, overwrite, lambda tmp: shutil.copy2(src_path, tmp))
    ...
    with torch.inference_mode():
        ...
        if output_mode == "file":
            if path_n is not None:
                ...
                if out_path is None:
                    _LOGGER.warning("postprocess: output=%r requires path to be a .h5/.hdf5 file. Got path=%r; falling back to output='memory'.", output, path)
                    output_mode = "memory"
                elif not _is_path_writable(out_path):
                    _LOGGER.warning("postprocess: output=%r path is not writable: %r; falling back to output='memory'.", output, out_path)
                    out_path = None
                    output_mode = "memory"
            else:
                output_mode = "memory"
        ...
        if (src.endswith(".h5") or src.endswith(".hdf5")) and os.path.isfile(src):
            if output_mode == "file":
                ...
                return copy_predictions_h5_atomic(os.fspath(src), os.fspath(out_path), overwrite=str(overwrite_mode or "replace"))
        ...
        if not os.path.isfile(pred_path):
            ...
            if count <= 0 or (not out_shape_t) or any(int(d) <= 0 for d in out_shape_t):
                raise ValueError(f"postprocess: invalid manifest metadata: count={count!r}, out_shape={out_shape!r}")
            store_float = _get_prediction_dtype(chunks_dir)
            concat_memory_mapped_tensor(os.fspath(chunks_dir), os.fspath(pred_path), count=count, out_shape=out_shape_t, store_float=store_float)
        ...
        if out_path is not None:
            if os.path.exists(out_path):
                if overwrite_mode == "resume" and os.path.isfile(out_path):
                    validate_predictions_h5(
                        ...
                        in_dim=(int(X_mmt.shape[1]) if hasattr(X_mmt, "shape") and len(X_mmt.shape) > 1 else None),
                    )
                    return PersistentTensorDict(filename=out_path, mode="r")
                if overwrite_mode == "error":
                    raise FileExistsError(f"postprocess: destination already exists: {out_path!r}")
            ...
            if not os.path.isfile(out_path):
                raise RuntimeError(f"postprocess: persistent output missing after write: {out_path!r}")
            validate_predictions_h5(
                os.fspath(out_path),
                out_shape=tuple(int(d) for d in Y_mmt.shape[1:]),
                in_dim=(int(X_mmt.shape[1]) if hasattr(X_mmt, "shape") and len(X_mmt.shape) > 1 else None),
            )
            remove_prediction_artifacts(memmap_dir=os.fspath(memmap_dir), pred_path=os.fspath(pred_path))
            return out_td
...
class _BatchSliceGetter:
    def __init__(self: Self, raw_X: Any, raw_Y: Any, *args: Any, features_only: bool) -> None:
        self.raw_X = raw_X
        self.raw_Y = raw_Y
...
class _KeyView(collections.abc.Mapping):
    ...
    def __init__(self: Self, data: Mapping[Any, Any], keys: Sequence[Any]) -> None:
        self._data = data
        self._keys = keys
...
class _KeyCursor:
    ...
    def __init__(self: Self, data: Mapping[Any, Any], keys: Optional[Sequence[Any]] = None) -> None:
        self._data = data
    ...
    def __call__(self: Self, s: int, e: int) -> _KeyView:
        ...
        if s_i == 0 and self._pos != 0:
            self._reset()
        if s_i != self._pos:
            raise RuntimeError(f"key_index_mapping_getters: non-sequential access requested (expected s={self._pos}, got s={s_i}).")
        need = e_i - s_i
        if need <= 0:
        ...
        for _ in range(int(need)):
            try:
                batch_keys.append(next(self._it))
...
class MappingSlicer:
    ...
    def __init__(self: Self, const_items: Mapping[Any, Any], slice_items: Tuple[Any, ...]) -> None:
...
class Unsharder:
    __slots__ = (
        "chunk_dir", "rank", "use_mmt_pred_parts", "cache", "pred_pool",
        "target_rows", "make_fence_event", "use_buffer", "rows_buf",
        "pred_buf", "pred_handle", "pred_buf_is_pinned", "buf_needs_wait_evt",
        "buf_wait_evt", "buf_wait_device", "buf_fill", "pending_rows",
        "pending_preds", "pending_count", "chunk_idx", "first_tail",
        "pending_tail", "variable_shape"
    )
    ...
    def flush(self: Self) -> None:
        ...
        rows_path = os.path.join(self.chunk_dir, f"part-r{self.rank:05d}-c{self.chunk_idx:06d}-rows.pt")
        pred_ext = "mmt" if self.use_buffer and self.use_mmt_pred_parts else "pt"
        pred_path = os.path.join(self.chunk_dir, f"part-r{self.rank:05d}-c{self.chunk_idx:06d}-pred.{pred_ext}")

        if self.cache is not None:
            self.cache.submit(rows, path=rows_path)
        else:
            save_temp(rows_path, rows)  # using atomic torch_save substitute wrapper
    ...
    def append(self: Self, rows_cpu: object, preds: object) -> None:
        ...
        rows_t = rows_cpu if isinstance(rows_cpu, torch.Tensor) else torch.as_tensor(rows_cpu, dtype=torch.int64)
        rows_t = rows_t.reshape(-1).to(dtype=torch.int64, device="cpu", copy=False)
        if rows_t.numel() != b:
            raise RuntimeError(f"infer: rows/preds batch mismatch rows={rows_t.numel()} preds={b}")
        ...
        if not self.use_buffer:
            if self.pending_tail is None:
                ...
                self.flush()
                self.pending_tail = tail
            self.pending_rows.append(rows_t.clone())
        ...
        if self.pred_buf is None:
            if self.pred_pool is not None:
                self.pred_buf, self.pred_handle = self.pred_pool.get((self.target_rows, *tail), dtype=preds.dtype, return_handle=True)
            else:
                self.pred_buf = torch.empty((self.target_rows, *tail), dtype=preds.dtype)
                self.pred_handle = None
            self.pred_buf_is_pinned = False
            with contextlib.suppress(Exception):
                if callable(is_pinned := getattr(self.pred_buf, "is_pinned", None)):
        ...
        while start < b:
            ...
            self.rows_buf[self.buf_fill : self.buf_fill + n].copy_(rows_t[start : start + n])
            non_blocking = bool(self.pred_buf_is_pinned) and preds.device.type != "cpu"
            self.pred_buf[self.buf_fill : self.buf_fill + n].copy_(preds[start : start + n], non_blocking=non_blocking)
            ...
            if non_blocking:
                ...
                try:
                    evt = None
                    if self.pred_pool is not None and self.pred_handle is not None:
                        if callable(fe := getattr(self.pred_pool, "fence_event", None)):
                            evt = fe(self.pred_handle, self.make_fence_event)
                    if evt is None and callable(self.make_fence_event):
                        evt = self.make_fence_event()
                    if evt is not None:
                        if callable(rec := getattr(evt, "record", None)):
                            try:
                                if preds.device.type == "cuda" and hasattr(torch, "cuda"):
                                    stream = torch.cuda.current_stream(device=preds.device)
                                    try: rec(stream)
                                    except TypeError: rec()
                                else:
                                    rec()
                            except Exception:
                                with contextlib.suppress(Exception): rec()
    ...
    def __call__(self: Self, batch: Any) -> Any:
        ...
        match batch:
            case list() | tuple() as seq:
                ...
                def _standardize_batch(items: Sequence[object]) -> Union[TensorDictBase, Dict[str, object]]:
                    if all(isinstance(elem, TensorDictBase) for elem in items):
                        return torch.stack(items, dim=0)

                    def _stack_key(key: str, default: object | None = None) -> Union[torch.Tensor, List[object], None]:
                        vals = [(x.get(key, default) if isinstance(x, Mapping) else default) for x in items]
                        if all(v is None for v in vals):
                            return None
                        return _to_safe_tensor(
                            torch.stack([_to_safe_tensor(v) for v in vals if v is not None])
                            if any(isinstance(v, torch.Tensor) for v in vals) else vals
                        )
                ...
                stacked = _standardize_batch(seq)
                if isinstance(stacked, TensorDictBase):
                    with contextlib.suppress(Exception):
                        canonicalize_keys_(stacked, allow_missing_labels=True)

                try:
                    conv = preprocess(stacked, flatten_features=flatten, labels_dtype=labels_dtype, sanitize=sanitize)
                except Exception:
                    return stacked
            ...
            case Mapping() as m:
                try:
                    conv = preprocess(m, flatten_features=flatten, labels_dtype=labels_dtype, sanitize=sanitize)
                except Exception:
                    conv = m
